...
function crossSuppressedBlock(sup) {
  ...
  return '<div style="border:1px solid rgba(var(--ink),.09); border-radius:6px; margin-top:14px; padding:14px 16px">'
    + '<div style="' + M + '; font-size:10.5px; letter-spacing:.14em; color:var(--warn)">MATCHED BUT NOT PRICED · ' + num(sup.total) + '</div>'
    + '<div style="font-size:12.5px; color:rgba(var(--ink),.62); margin-top:7px; line-height:1.6">'
    ...
    + (beispiele.length
      ? '<div style="margin-top:9px">' + beispiele.map((b) =>
        '<div style="' + M + '; font-size:10.5px; color:rgba(var(--ink),.5); margin-top:5px; line-height:1.5">'
...
export function renderCross(T) {
  ...
  return '<div>'
    ...
    + '</div></div>'
    + '<div style="font-size:13px; color:rgba(var(--ink),.55); margin-top:10px; max-width:760px">Matched by title similarity, not by ticker. ' + esc(gateNote) + '. GAP is the distance between the two mid prices, and nobody trades a mid. NET OF FEES prices the basket that would capture it (buy the yes side at the ask, buy the other side at the other venue&#39;s ask) and subtracts both venues&#39; taker fee curves. The top ' + num(cl.depth_rows || 12) + ' rows by net are re-quoted against both order books, so their number holds for the size shown beneath it; the rest price the touch at the fee clip of 100 and say so. Settlement rules and resolution sources still differ, and two matched titles can still be two different questions (studies 08 and 11).</div>'
    ...
    + cRows.map((c) => {
      ...
      return '<div style="display:grid; grid-template-columns:1fr 104px 104px 84px 108px 124px 112px; align-items:center; padding:13px 24px; border-bottom:1px solid rgba(var(--ink),.06)">'
        ...
        + '<div style="' + M + '; font-size:13px; color:' + netFarbe + '">' + netLabel + '</div>'
        ...
        + (sizeLabel ? '<div style="' + M + '; font-size:9.5px; color:rgba(var(--ink),.5); margin-top:2px; line-height:1.3">' + esc(sizeLabel) + '</div>' : '')
        + '</div></div>';
    }).join('')
