...
function asOfLine(iso) {
  return iso ? '<span style="' + M + '; font-size:11px; color:rgba(234,230,220,.55)">as of ' + esc(stempel(iso)) + '</span>' : '';
}
...
function marketRowHtml(v) {
  return '<div ' + v.act + ' class="hv-panel" style="display:grid; grid-template-columns:' + MARKT_SPALTEN + '; align-items:center; padding:13px 24px; border-bottom:1px solid rgba(234,230,220,.06); cursor:pointer">'
    + '<div style="padding-right:20px">'
    + '<div style="font-size:13.5px; line-height:1.35">' + esc(v.title) + '</div>'
    + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); margin-top:3px">' + esc(v.meta) + '</div></div>'
    + '<div style="' + M + '; font-size:15px; text-align:right">' + v.priceLabel + '</div>'
    + '<div style="' + v.changeStyle + '">' + v.changeLabel + '</div>'
    + '<div style="' + M + '; font-size:12px; text-align:right; color:rgba(234,230,220,.55)">' + esc(v.spreadLabel || '—') + '</div>'
    + '<div style="' + M + '; font-size:12px; text-align:right; color:rgba(234,230,220,.55)">' + esc(v.liqLabel || '—') + '</div>'
    + '<div style="' + M + '; font-size:13px; text-align:right">' + v.volLabel + '</div>'
    + '<div style="' + M + '; font-size:12px; text-align:right; color:rgba(234,230,220,.55)">' + esc(v.ends) + '</div></div>';
}
...
function insightPanel(titel, sub, rowsHtml, leerSatz) {
  return '<div style="background:#131311; border:1px solid rgba(234,230,220,.09); border-radius:6px; overflow:hidden">'
    + '<div style="padding:9px 14px; border-bottom:1px solid rgba(234,230,220,.08)"><h2 style="' + M + '; font-size:11px; letter-spacing:.13em; color:#D9A648; margin:0; display:inline">' + titel + '</h2>'
    + (sub ? ' <span style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.55); letter-spacing:0">· ' + sub + '</span>' : '') + '</div>'
    + (rowsHtml || '<div style="padding:12px 14px; ' + M + '; font-size:10.5px; color:rgba(234,230,220,.55)">' + esc(leerSatz) + '</div>')
    + '</div>';
}
...
const VERDICT_TAG = {
  ja: { label: 'CONFIRMED', color: '#D9A648', bg: 'rgba(217,166,72,.12)' },
  nein: { label: 'REFUTED', color: '#DE8A7A', bg: 'rgba(217,95,82,.12)' },
  offen: { label: 'NOT IDENTIFIED', color: '#DE7E36', bg: 'rgba(222,126,54,.12)' },
  kontrolle: { label: 'CONTROL', color: '#6E9BC8', bg: 'rgba(110,155,200,.14)' }
};
...
function verdictTag(art) {
  ...
  const style = M + '; font-size:10.5px; letter-spacing:.12em; border-radius:4px; padding:3px 8px; white-space:nowrap; '
    + (v ? 'color:' + v.color + '; background:' + v.bg : 'color:rgba(234,230,220,.55); border:1px solid rgba(234,230,220,.16)');
...
function sectionHead(label, right, color) {
  return '<div style="display:flex; align-items:center; justify-content:space-between; gap:12px; padding:12px 24px; border-bottom:1px solid rgba(234,230,220,.09); background:#131311">'
    + '<div style="' + M + '; font-size:11px; letter-spacing:.16em; color:' + (color || '#D9A648') + '">' + label + '</div>'
...
export function renderOverview(T) {
  ...
  const link = (i, label, color) => (i >= 0
    ? '<div ' + goStudy(i) + ' class="hv-accent" style="' + M + '; font-size:11px; color:' + (color || 'rgba(234,230,220,.6)') + '; cursor:pointer; padding:5px 0; white-space:nowrap">' + label + ' →</div>'
    : '');
  ...
  let board;
  if (studien.length) {
    board = '<div style="display:grid; grid-template-columns:36px 1fr 128px 190px 150px; padding:9px 24px; border-bottom:1px solid rgba(234,230,220,.09); ' + HEAD_CELL + '">'
      ...
      + studien.map((st, i) => {
        ...
        return '<div ' + goStudy(microIdx, studieAnker(st, i)) + ' class="hv-panel" style="display:grid; grid-template-columns:36px 1fr 128px 190px 150px; align-items:center; padding:11px 24px; border-bottom:1px solid rgba(234,230,220,.06); cursor:pointer">'
          + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6)">' + String(i + 1).padStart(2, '0') + '</div>'
          ...
          + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); margin-top:3px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis" title="' + esc(String(st.verdikt || '')) + '">' + esc(String(st.verdikt || '').split('. ')[0]) + '</div></div>'
          + '<div>' + verdictTag(st.verdikt_art) + '</div>'
          + '<div style="text-align:right; min-width:0"><div style="' + M + '; font-size:14px">' + esc(kn.value) + (kn.unit ? ' <span style="font-size:11px; color:rgba(234,230,220,.6)">' + esc(kn.unit) + '</span>' : '') + '</div>'
          + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis" title="' + esc(kn.label + (nLabel ? ' · ' + nLabel : '')) + '">' + esc(kn.label) + (nLabel ? ' · ' + esc(nLabel) : '') + '</div></div>'
          + '<div style="' + M + '; font-size:11px; text-align:right; color:rgba(234,230,220,.5)">' + esc(basis.fenster || '—') + '</div></div>';
      }).join('');
  } else {
  ...
  const notesStrip = noteList.length
    ? noteList.map((n) =>
      '<div ' + goStudy(notesIdx) + ' class="hv-panel" style="display:grid; grid-template-columns:96px 1fr 120px; gap:12px; align-items:center; padding:11px 24px; border-bottom:1px solid rgba(234,230,220,.06); cursor:pointer">'
      + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6)">' + esc(String(n.date || '').slice(0, 10)) + '</div>'
      + '<div style="font-size:13px; line-height:1.35">' + esc(n.title || '—') + '</div>'
      + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); text-align:right">' + esc(n.venue || '') + '</div></div>'
    ).join('')
  ...
  const liveRow = '<div style="display:grid; grid-template-columns:repeat(3,1fr); border-bottom:1px solid rgba(234,230,220,.09)">'
  ...
  const pfade = '<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:12px; margin-top:16px; max-width:860px">'
    + pfadKarte(goStudy(runsIdx), 'TESTED STRATEGY', '#D9A648', 'Researched, then run with real money: every bet, its latency and the on-chain wallet that proves it.')
    + pfadKarte(T.act(() => T.go('markets')), 'ANALYSIS TOOL', '#6E9BC8', 'Live screens on Polymarket & Kalshi: markets, tape, whale flow, cross-venue, risk.')
    + '</div>';
  return '<div>'
    + '<div style="padding:24px 24px 20px; border-bottom:1px solid rgba(234,230,220,.09)">'
    + '<h1 style="font-size:26px; line-height:1.2; margin:0; font-weight:600; letter-spacing:-0.015em">Prediction-market microstructure, <em style="color:#D9A648">measured on self-recorded books.</em></h1>'
    + '<div style="font-size:14px; color:rgba(234,230,220,.66); margin-top:8px; max-width:760px">' + esc(subline) + '</div>'
    + pfade
    ...
    + (pilotIdx >= 0 ? '<span ' + goStudy(pilotIdx) + ' class="hv-accent" style="color:rgba(234,230,220,.55); cursor:pointer; display:inline-block; padding:5px 0">Pre-registered pilot →</span>' : '')
    + '</div></div>'
    ...
    + sectionHead('VERDICT BOARD · ' + (studien.length ? num(counts.total) + ' STUDIES' : 'MICROSTRUCTURE'),
      (micro && micro.stand_utc ? '<span style="' + M + '; font-size:11px; color:rgba(234,230,220,.55)">payload ' + esc(stempel(micro.stand_utc)) + '</span>' : '')
      + link(microIdx, 'FULL REPORT'))
    + board

    + sectionHead('FIELD NOTES', link(notesIdx, 'ALL NOTES'), '#6E9BC8')
    + notesStrip

    + sectionHead('ANALYSIS TOOL · LIVE DATA', asOfLine(s.liveAsOf) + '<div ' + T.act(() => T.go('markets')) + ' class="hv-accent" style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); cursor:pointer; padding:5px 0">MARKETS →</div><div ' + T.act(() => T.go('flow')) + ' class="hv-accent" style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); cursor:pointer; padding:5px 0">TAPE →</div>')
    + liveRow
    + '<div style="padding:22px 24px; text-align:center; ' + M + '; font-size:11px; color:rgba(234,230,220,.55)">Public data only · live blocks refresh every 30 seconds · research payloads are frozen files under ./data</div>'
    + '</div>';
}
...
function pfadKarte(act, label, farbe, satz) {
  return '<div ' + act + (act ? ' class="hv-panel"' : '') + ' style="background:#131311; border:1px solid rgba(234,230,220,.09); border-radius:6px; padding:14px 16px' + (act ? '; cursor:pointer' : '') + '">'
    + '<div style="' + M + '; font-size:11px; letter-spacing:.16em; color:' + farbe + '">' + label + ' →</div>'
    + '<div style="font-size:12.5px; color:rgba(234,230,220,.6); margin-top:6px; line-height:1.5">' + satz + '</div></div>';
}

function kpiCell(label, value, sub, borderRight, signed) {
  const color = signed == null ? '#EAE6DC' : (+signed >= 0 ? '#69B47E' : '#D95F52');
  return '<div style="padding:16px 20px' + (borderRight ? '; border-right:1px solid rgba(234,230,220,.09)' : '') + '">'
    ...
    + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); margin-top:4px">' + sub + '</div></div>';
}
...
export function renderMarkets(T) {
  const s = T.state;
  if (!T.markets.length) {
    return '<div>' + seitenKopf('MARKETS', 'Every market, one table', '#D9A648')
  ...
  const zeile = (m, mitte, rechts) =>
    '<div ' + T.act(() => T.openMarket(m.id)) + ' class="hv-panel" style="display:grid; grid-template-columns:1fr 46px 78px; gap:8px; align-items:center; padding:8px 14px; border-bottom:1px solid rgba(234,230,220,.05); cursor:pointer">'
    + '<div style="font-size:12px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis" title="' + esc(m.title) + '">' + esc(m.title) + '</div>'
    + '<div style="' + M + '; font-size:11.5px; text-align:right; color:rgba(234,230,220,.55)">' + mitte + '</div>'
    + '<div style="' + M + '; font-size:11.5px; text-align:right">' + rechts + '</div></div>';
  const kpiStrip = '<div style="display:grid; grid-template-columns:repeat(4,1fr); border:1px solid rgba(234,230,220,.09); border-radius:6px; margin-top:14px; overflow:hidden; background:#131311">'
  ...
  const einblicke = '<div style="display:grid; grid-template-columns:repeat(auto-fit, minmax(280px, 1fr)); gap:12px; margin-top:12px">'
    + insightPanel('TOP MOVERS · 1D', 'yes price · change',
      movers.map((m) => zeile(m, m.yes + '¢', '<span style="color:' + (m.chg >= 0 ? '#69B47E' : '#D95F52') + '">' + (m.chg >= 0 ? '+' : '') + m.chg + '¢</span>')).join(''),
  ...
  const badge = mActive.length ? M + '; font-size:11px; color:#0B0B0A; background:#D9A648; border-radius:4px; padding:1px 7px' : 'display:none';
  const chevron = M + '; font-size:16px; color:rgba(234,230,220,.5); transition:transform .18s ease; transform:rotate(' + (s.marketFiltersOpen ? '90deg' : '0deg') + ')';
  ...
  return '<div>'
    ...
    + '<div><div style="' + M + '; font-size:11px; letter-spacing:.18em; color:#D9A648">MARKETS</div>'
    ...
    + '<input value="' + esc(s.marketQuery) + '" ' + T.inp((e) => T.setState({ marketQuery: e.target.value }), 'marketQuery') + ' placeholder="Search markets…" style="background:#131311; border:1px solid rgba(234,230,220,.35); border-radius:4px; padding:9px 12px; ' + M + '; font-size:12.5px; color:#EAE6DC; width:230px" />'
    + '<div ' + T.act(() => T.setState({ mPlatform: 'all', mStatus: 'active', mProb: 'all', mLiq: 'all', mVol: 'all', mEnds: 'all', mAge: 'all', mExclude: [], marketCat: 'All', marketQuery: '', mQuick: 'trending', marketSort: 'volume' })) + ' class="hv-bd32" style="font-size:12.5px; color:rgba(234,230,220,.6); border:1px solid rgba(234,230,220,.16); border-radius:4px; padding:9px 13px; cursor:pointer">Reset filters</div>'
    + '</div></div>'
    ...
    + '<div ' + T.act(() => T.setState({ marketFiltersOpen: !s.marketFiltersOpen })) + ' class="hv-el" style="display:flex; align-items:center; justify-content:space-between; padding:11px 15px; background:#131311; cursor:pointer">'
    ...
    + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6)">' + mRows.length + ' of ' + T.markets.length + ' markets</div>'
    + mActive.map((c) =>
      '<div ' + T.act(() => T.setState(c.reset)) + ' class="hv-bd30" style="' + M + '; font-size:11px; color:rgba(234,230,220,.7); border:1px solid rgba(234,230,220,.14); background:#1A1A17; border-radius:4px; padding:3px 9px; cursor:pointer">' + esc(c.label) + ' ✕</div>'
    ).join('')
    + '</div></div>'
    ...
    + '<div style="display:grid; grid-template-columns:' + MARKT_SPALTEN + '; align-items:center; padding:10px 24px; border-bottom:1px solid rgba(234,230,220,.09); background:#131311; position:sticky; top:0; z-index:3; ' + HEAD_CELL + '">'
    + '<div>MARKET</div>'
    + '<div style="text-align:right">YES</div>'
    + '<div ' + T.act(() => T.setState({ marketSort: 'change' })) + ' aria-pressed="' + (s.marketSort === 'change' ? 'true' : 'false') + '"' + ' style="text-align:right; cursor:pointer; padding:5px 0; color:' + (s.marketSort === 'change' ? '#D9A648' : 'rgba(234,230,220,.6)') + '">CHANGE 1D</div>'
    + '<div style="text-align:right">SPREAD</div>'
    + '<div ' + T.act(() => T.setState({ marketSort: 'liquidity' })) + ' aria-pressed="' + (s.marketSort === 'liquidity' ? 'true' : 'false') + '"' + ' style="text-align:right; cursor:pointer; padding:5px 0; color:' + (s.marketSort === 'liquidity' ? '#D9A648' : 'rgba(234,230,220,.6)') + '">LIQUIDITY</div>'
    + '<div ' + T.act(() => T.setState({ marketSort: 'volume' })) + ' aria-pressed="' + (s.marketSort === 'volume' ? 'true' : 'false') + '"' + ' style="text-align:right; cursor:pointer; padding:5px 0; color:' + (s.marketSort === 'volume' ? '#D9A648' : 'rgba(234,230,220,.6)') + '">VOLUME 24H</div>'
    + '<div ' + T.act(() => T.setState({ marketSort: 'ending' })) + ' aria-pressed="' + (s.marketSort === 'ending' ? 'true' : 'false') + '"' + ' style="text-align:right; cursor:pointer; padding:5px 0; color:' + (s.marketSort === 'ending' ? '#D9A648' : 'rgba(234,230,220,.6)') + '">RESOLVES</div></div>'
    + mRows.map((m) => marketRowHtml(Object.assign(T.marketView(m), {
      spreadLabel: mx(m).spread != null ? mx(m).spread + '¢' : '—',
      liqLabel: m.liq ? money(m.liq) : '—'
    }))).join('')
    + (mRows.length === 0 ? '<div style="padding:60px; text-align:center; ' + M + '; font-size:12px; color:rgba(234,230,220,.55)">No market matches that filter.</div>' : '')
    + '</div>';
}
...
export function renderFlow(T) {
  const s = T.state;
  if (!T.tape.length) {
    return '<div>' + seitenKopf('LIVE TAPE', 'Every large print as it lands', '#D9A648')
  ...
  return '<div>'
    + '<div style="padding:20px 24px 14px; border-bottom:1px solid rgba(234,230,220,.09)">'
    + '<div style="display:flex; align-items:flex-end; justify-content:space-between; gap:20px">'
    + '<div><div style="' + M + '; font-size:11px; letter-spacing:.18em; color:#D9A648">LIVE TAPE</div>'
    ...
    + '<input value="' + esc(s.tapeQuery) + '" ' + T.inp((e) => T.setState({ tapeQuery: e.target.value }), 'tapeQuery') + ' placeholder="market, wallet, trader…" style="background:#131311; border:1px solid rgba(234,230,220,.35); border-radius:4px; padding:9px 12px; ' + M + '; font-size:12.5px; color:#EAE6DC; width:250px" />'
    + '</div></div>'
    ...
    + '<div style="display:grid; grid-template-columns:repeat(4,1fr); border-bottom:1px solid rgba(234,230,220,.09)">'
    + kpis.map((k, i) =>
      '<div style="padding:14px 24px' + (i < 3 ? '; border-right:1px solid rgba(234,230,220,.09)' : '') + '">'
      + '<div style="' + HEAD_CELL + '">' + k.label + '</div>'
      + '<div style="' + M + '; font-size:22px; margin-top:7px">' + k.value + '</div></div>'
    ).join('')
    + '</div>'

    + '<div style="display:grid; grid-template-columns:96px 160px 1fr 110px 84px 90px 110px 96px; padding:10px 24px; border-bottom:1px solid rgba(234,230,220,.09); background:#131311; position:sticky; top:0; z-index:3; ' + HEAD_CELL + '">'
    ...
    + tapeFiltered.map((t0) => {
      ...
      return '<div ' + (klickbar ? t.act + ' class="hv-panel"' : '') + ' style="display:grid; grid-template-columns:96px 160px 1fr 110px 84px 90px 110px 96px; align-items:center; padding:12px 24px; border-bottom:1px solid rgba(234,230,220,.06); ' + M + '; font-size:12.5px; ' + (klickbar ? 'cursor:pointer; ' : '') + '">'
        + '<div style="color:rgba(234,230,220,.55)">' + esc(t.ago) + '</div>'
        ...
        + '<div style="font-size:11px; color:rgba(234,230,220,.55)">' + esc(t.category || 'Other') + '</div>'
        + '<div style="' + t.sideStyle + '">' + esc(t.side) + '</div>'
        + '<div style="text-align:right">' + esc(t.price) + '</div>'
        + '<div style="text-align:right">' + t.size + '</div>'
        + '<div style="text-align:right; color:rgba(234,230,220,.6); font-size:11px">' + esc(t.venue) + '</div></div>';
    }).join('')
    + '</div>';
}
...
function crossGateBlock(T) {
  ...
  return '<div style="padding:26px 24px">'
    ...
    + '<div style="' + M + '; font-size:11px; letter-spacing:.14em; color:#DE7E36">NO PAIR CLEARS THE GATE</div>'
    + '<div style="font-size:13px; color:rgba(234,230,220,.6); margin-top:9px; line-height:1.6">No cross-venue pair clears the match gate right now (' + CROSS_GATE_TEXT + '). '
    + 'See studies 08 and 11: the two 79¢/64¢ \'edges\' were mismatched questions.</div>'
    + (microIdx >= 0 && T.goStudy
      ? '<div ' + T.act(() => T.goStudy(microIdx)) + ' class="hv-accent" style="' + M + '; font-size:11px; color:#6E9BC8; margin-top:12px; cursor:pointer">Open the microstructure report (#research/microstructure) →</div>'
...
export function renderCross(T) {
  ...
  if (!T.crossPairs.length) {
    ...
    if (!hk) {
      body = '<div style="padding:26px 24px">'
        ...
        + '<div style="' + M + '; font-size:11px; letter-spacing:.12em; color:#DE7E36">MATCHING PAIRS ACROSS VENUES…</div></div>'
      ...
      body = leerBlock('NO PAIRS', herkunftSatz(hk, '/api/cross'))
        + (T.neuLaden ? '<div style="padding:0 24px 24px"><div ' + T.act(() => T.neuLaden('cross', 'cross')) + ' class="hv-bd32" style="display:inline-block; ' + M + '; font-size:11px; color:rgba(234,230,220,.7); border:1px solid rgba(234,230,220,.16); border-radius:4px; padding:6px 12px; cursor:pointer">Try again</div></div>' : '');
    } else {
      body = crossGateBlock(T);
    }
    return '<div>' + seitenKopf('CROSS-VENUE', 'The same question, two prices', '#6E9BC8') + body + '</div>';
  }
  ...
  const stepGroup = (label, valueLabel, onDown, onUp) =>
    ...
    + '<div ' + T.act(onDown) + ' class="hv-bd35w" style="width:26px; height:30px; flex:none; border:1px solid rgba(234,230,220,.16); border-radius:4px; display:flex; align-items:center; justify-content:center; ' + M + '; font-size:13px; color:rgba(234,230,220,.7); cursor:pointer">−</div>'
    + '<div style="flex:1; background:#131311; border:1px solid rgba(234,230,220,.16); border-radius:4px; padding:6px 8px; ' + M + '; font-size:12px; text-align:center">' + esc(valueLabel) + '</div>'
    + '<div ' + T.act(onUp) + ' class="hv-bd35w" style="width:26px; height:30px; flex:none; border:1px solid rgba(234,230,220,.16); border-radius:4px; display:flex; align-items:center; justify-content:center; ' + M + '; font-size:13px; color:rgba(234,230,220,.7); cursor:pointer">+</div>'
    + '</div></div>';

  return '<div>'
    + '<div style="padding:20px 24px 14px; border-bottom:1px solid rgba(234,230,220,.09)">'
    + '<div style="display:flex; align-items:flex-end; justify-content:space-between; gap:20px">'
    + '<div><div style="' + M + '; font-size:11px; letter-spacing:.18em; color:#6E9BC8">CROSS-VENUE</div>'
    ...
    + '<div style="display:flex; align-items:center; gap:10px">'
    + asOfLine(cl.as_of)
    + '<input value="' + esc(s.crossQuery) + '" ' + T.inp((e) => T.setState({ crossQuery: e.target.value }), 'crossQuery') + ' placeholder="bitcoin, fed, election…" style="background:#131311; border:1px solid rgba(234,230,220,.35); border-radius:4px; padding:9px 12px; ' + M + '; font-size:12.5px; color:#EAE6DC; width:230px" />'
    ...
    + '<div style="border:1px solid rgba(234,230,220,.09); border-radius:6px; margin-top:14px; padding:16px; display:grid; grid-template-columns:repeat(4, minmax(0,1fr)); gap:16px 18px">'
    ...
    + '<div style="display:flex; align-items:center; gap:8px; margin-top:12px; flex-wrap:wrap">'
    + activeChips.map((c) => '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.7); border:1px solid rgba(234,230,220,.14); background:#1A1A17; border-radius:4px; padding:3px 9px">' + esc(c) + '</div>').join('')
    + '</div></div>'

    + '<div style="display:grid; grid-template-columns:repeat(4,1fr); border-bottom:1px solid rgba(234,230,220,.09)">'
    + '<div style="padding:14px 24px; border-right:1px solid rgba(234,230,220,.09)"><div style="' + HEAD_CELL + '">PAIRS SHOWN</div><div style="' + M + '; font-size:22px; margin-top:7px">' + cRows.length + '</div></div>'
    + '<div style="padding:14px 24px; border-right:1px solid rgba(234,230,220,.09)"><div style="' + HEAD_CELL + '">LARGEST GAP</div><div style="' + M + '; font-size:22px; margin-top:7px; color:#DE7E36">' + (gaps.length ? gaps[gaps.length - 1] + '¢' : '—') + '</div></div>'
    + '<div style="padding:14px 24px; border-right:1px solid rgba(234,230,220,.09)"><div style="' + HEAD_CELL + '">MEDIAN SIMILARITY</div><div style="' + M + '; font-size:22px; margin-top:7px">' + (medianSim ? medianSim.toFixed(2) : '—') + '</div></div>'
    ...
    + '<div style="display:grid; grid-template-columns:1fr 118px 118px 96px 110px 118px; padding:10px 24px; border-bottom:1px solid rgba(234,230,220,.09); background:#131311; position:sticky; top:0; z-index:3; ' + HEAD_CELL + '">'
    ...
    + cRows.map((c) => {
      ...
      const gapStyle = M + '; font-size:14px; text-align:right; color:' + (g >= 5 ? '#DE7E36' : g >= 3 ? '#EAE6DC' : 'rgba(234,230,220,.5)');
      return '<div style="display:grid; grid-template-columns:1fr 118px 118px 96px 110px 118px; align-items:center; padding:13px 24px; border-bottom:1px solid rgba(234,230,220,.06)">'
        + '<div style="padding-right:20px"><div style="font-size:13.5px; line-height:1.35">' + esc(c.event) + '</div>'
        + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); margin-top:3px">' + esc(c.cat) + ' · similarity ' + c.sim.toFixed(2) + '</div></div>'
        + '<div style="' + M + '; font-size:14px; text-align:right; color:#D9A648">' + c.pm + '¢</div>'
        + '<div style="' + M + '; font-size:14px; text-align:right; color:#6E9BC8">' + c.ks + '¢</div>'
        + '<div style="' + gapStyle + '">' + g + '¢</div>'
        + '<div style="' + M + '; font-size:12.5px; text-align:right; color:rgba(234,230,220,.6)">' + money(c.pmVol + c.ksVol) + '</div>'
        + '<div style="' + M + '; font-size:12px; text-align:right; color:rgba(234,230,220,.6)">' + esc(c.held) + '</div></div>';
    }).join('')
    + (cRows.length === 0 ? '<div style="padding:60px; text-align:center; ' + M + '; font-size:12px; color:rgba(234,230,220,.55)">No pair passes the local filters; loosen a stepper above.</div>' : '')
    + '</div>';
}
...
export function renderResolved(T) {
  ...
  if (!resAll.length) {
    return '<div>' + seitenKopf('RESOLVED', 'How the last questions ended', '#D9A648')
  ...
  return '<div>'
    + '<div style="padding:20px 24px 14px; border-bottom:1px solid rgba(234,230,220,.09)">'
    + '<div style="display:flex; align-items:flex-end; justify-content:space-between; gap:20px">'
    + '<div><div style="' + M + '; font-size:11px; letter-spacing:.18em; color:#D9A648">RESOLVED</div>'
    ...
    + '<input value="' + esc(s.resQuery) + '" ' + T.inp((e) => T.setState({ resQuery: e.target.value }), 'resQuery') + ' placeholder="Search resolved markets…" style="background:#131311; border:1px solid rgba(234,230,220,.35); border-radius:4px; padding:9px 12px; ' + M + '; font-size:12.5px; color:#EAE6DC; width:250px" />'
    + '</div>'
    + '<div style="font-size:13px; color:rgba(234,230,220,.55); margin-top:10px; max-width:700px">The last price before settlement next to the answer. The gap between the two is what the crowd got wrong.</div>'
    + '<div style="display:grid; grid-template-columns:repeat(4, minmax(0,1fr)); gap:14px 18px; margin-top:14px">'
    ...
    + kpis.map((k, i) =>
      '<div style="padding:14px 24px' + (i < 3 ? '; border-right:1px solid rgba(234,230,220,.09)' : '') + '">'
      + '<div style="' + HEAD_CELL + '">' + k.label + '</div>'
      + '<div style="' + M + '; font-size:22px; margin-top:7px; color:' + (k.amber ? '#DE7E36' : '#EAE6DC') + '">' + k.value + '</div></div>'
    ).join('')
    + '</div>'

    + '<div style="display:grid; grid-template-columns:1fr 110px 118px 128px 110px 120px; padding:10px 24px; border-bottom:1px solid rgba(234,230,220,.09); background:#131311; position:sticky; top:0; z-index:3; ' + HEAD_CELL + '">'
    ...
    + resRows.map((r) => {
      const answerStyle = M + '; font-size:11px; letter-spacing:.1em; border-radius:4px; padding:3px 10px; ' + (r.yes ? 'color:#0B0B0A; background:#D9A648' : 'color:#DE8A7A; border:1px solid rgba(217,95,82,.35)');
      const errStyle = M + '; font-size:14px; text-align:right; color:' + (r.err >= 50 ? '#D95F52' : r.err >= 25 ? '#DE7E36' : 'rgba(234,230,220,.6)');
      return '<div style="display:grid; grid-template-columns:1fr 110px 118px 128px 110px 120px; align-items:center; padding:13px 24px; border-bottom:1px solid rgba(234,230,220,.06)">'
        + '<div style="padding-right:20px"><div style="font-size:13.5px; line-height:1.35">' + esc(r.title) + '</div>'
        + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); margin-top:3px">' + esc(r.meta) + '</div></div>'
        ...
        + '<div style="' + M + '; font-size:12.5px; text-align:right; color:rgba(234,230,220,.6)">' + esc(r.vol) + '</div>'
        + '<div style="' + M + '; font-size:12px; text-align:right; color:rgba(234,230,220,.6)">' + esc(r.when) + '</div></div>';
    }).join('')
    + '</div>';
}
