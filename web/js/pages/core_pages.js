...
function asOfLine(iso) {
  return iso ? '<span style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.55)">as of ' + esc(stempel(iso)) + '</span>' : '';
}
...
function marketRowHtml(v) {
  return '<div ' + v.act + ' class="hv-panel" style="display:grid; grid-template-columns:' + MARKT_SPALTEN + '; align-items:center; padding:13px 24px; border-bottom:1px solid rgba(var(--ink),.06); cursor:pointer">'
    ...
    + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); margin-top:3px">' + esc(v.meta) + '</div></div>'
    + '<div style="' + M + '; font-size:var(--t-lead); text-align:right">' + v.priceLabel + '</div>'
    + '<div style="' + v.changeStyle + '">' + v.changeLabel + '</div>'
    + '<div style="' + M + '; font-size:var(--t-small); text-align:right; color:rgba(var(--ink),.55)">' + esc(v.spreadLabel || '—') + '</div>'
    + '<div style="' + M + '; font-size:var(--t-small); text-align:right; color:rgba(var(--ink),.55)">' + esc(v.liqLabel || '—') + '</div>'
    ...
    + (v.volShare != null
      ...
      : '')
    + '</div>'
    + '<div style="' + M + '; font-size:var(--t-small); text-align:right; color:rgba(var(--ink),.55)">' + esc(v.ends) + '</div></div>';
}
...
function insightPanel(titel, sub, rowsHtml, leerSatz) {
  return '<div style="background:var(--panel); border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); overflow:hidden">'
    + '<div style="padding:9px 14px; border-bottom:1px solid rgba(var(--ink),.08)"><h2 style="' + M + '; font-size:var(--t-micro); letter-spacing:.13em; color:var(--accent); margin:0; display:inline">' + titel + '</h2>'
    + (sub ? ' <span style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.55); letter-spacing:0">· ' + sub + '</span>' : '') + '</div>'
    + (rowsHtml || '<div style="padding:12px 14px; ' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.55)">' + esc(leerSatz) + '</div>')
    + '</div>';
}
...
function tapeLivePanel(T) {
  ...
  return '<div style="display:flex; align-items:baseline; justify-content:space-between; gap:10px; padding:14px 24px 8px; border-top:1px solid rgba(var(--ink),.06)">'
    + '<div style="' + M + '; font-size:var(--t-micro); letter-spacing:.15em; color:rgba(var(--ink),.6)">THE TAPE, LIVE · PRINTS ≥ $2.5K</div>'
...
function wireStrip(T) {
  ...
  return '<div aria-hidden="true" style="border-bottom:1px solid rgba(var(--ink),.09); background:var(--panel); overflow:hidden; ' + M + '; font-size:var(--t-small)">'
...
export function renderOverview(T) {
  ...
  const link = (i, label, color) => (i >= 0
    ? '<div ' + goStudy(i) + ' class="hv-accent" style="' + M + '; font-size:var(--t-micro); color:' + (color || 'rgba(var(--ink),.6)') + '; cursor:pointer; padding:5px 0; white-space:nowrap">' + label + ' →</div>'
    : '');
    ...
    board = '<div style="display:grid; grid-template-columns:36px 1fr 128px 190px 150px; padding:9px 24px; border-bottom:1px solid rgba(var(--ink),.09); ' + HEAD_CELL + '">'
      ...
      + studien.map((st, i) => {
        ...
        return '<div ' + goStudy(microIdx, studieAnker(st, i)) + ' class="hv-panel" style="display:grid; grid-template-columns:36px 1fr 128px 190px 150px; align-items:center; padding:11px 24px; border-bottom:1px solid rgba(var(--ink),.06); cursor:pointer">'
          + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6)">' + String(i + 1).padStart(2, '0') + '</div>'
          ...
          + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); margin-top:3px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis" title="' + esc(String(st.verdikt || '')) + '">' + esc(String(st.verdikt || '').split('. ')[0]) + '</div></div>'
          + '<div>' + verdictTag(st.verdikt_art) + '</div>'
          + '<div style="text-align:right; min-width:0"><div style="' + M + '; font-size:var(--t-body)">' + esc(kn.value) + (kn.unit ? ' <span style="font-size:var(--t-micro); color:rgba(var(--ink),.6)">' + esc(kn.unit) + '</span>' : '') + '</div>'
          + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis" title="' + esc(kn.label + (nLabel ? ' · ' + nLabel : '')) + '">' + esc(kn.label) + (nLabel ? ' · ' + esc(nLabel) : '') + '</div></div>'
  ...
  const notesStrip = noteList.length
    ? noteList.map((n) =>
      ...
      + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6)">' + esc(String(n.date || '').slice(0, 10)) + '</div>'
      + '<div style="font-size:var(--t-body); line-height:1.35">' + esc(n.title || '—') + '</div>'
      + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); text-align:right">' + esc(n.venue || '') + '</div></div>'
    ).join('')
  ...
  return '<div>'
    ...
    + '<div style="font-size:var(--t-body); color:rgba(var(--ink),.66); margin-top:8px; max-width:760px">' + esc(subline) + '</div>'
    + pfade
    ...
    + sectionHead('VERDICT BOARD · ' + (studien.length ? num(counts.total) + ' STUDIES' : 'MICROSTRUCTURE'),
      (micro && micro.stand_utc ? '<span style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.55)">payload ' + esc(stempel(micro.stand_utc)) + '</span>' : '')
    ...
    + sectionHead('ANALYSIS TOOL · LIVE DATA', asOfLine(s.liveAsOf) + '<div ' + T.act(() => T.go('markets')) + ' class="hv-accent" style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); cursor:pointer; padding:5px 0">MARKETS →</div><div ' + T.act(() => T.go('flow')) + ' class="hv-accent" style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); cursor:pointer; padding:5px 0">TAPE →</div>')
    + liveRow
    ...
    + caveatZeile('score_generic', {
      ...
      stil: 'padding:22px 24px; text-align:center; ' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.55); line-height:1.7'
    })
    + '</div>';
}
...
function pfadKarte(act, label, farbe, satz) {
  return '<div ' + act + (act ? ' class="hv-panel"' : '') + ' style="background:var(--panel); border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); padding:14px 16px' + (act ? '; cursor:pointer' : '') + '">'
    ...
    + '<div style="font-size:var(--t-small); color:rgba(var(--ink),.6); margin-top:6px; line-height:1.5">' + satz + '</div></div>';
}
...
function kpiCell(label, value, sub, borderRight, signed) {
  ...
  return '<div style="padding:16px 20px' + (borderRight ? '; border-right:1px solid rgba(var(--ink),.09)' : '') + '">'
    ...
    + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); margin-top:4px">' + sub + '</div></div>';
}
...
export function renderMarkets(T) {
  ...
  const zeile = (m, mitte, rechts) =>
    ...
    + '<div style="' + M + '; font-size:var(--t-small); text-align:right; color:rgba(var(--ink),.55)">' + mitte + '</div>'
  ...
  return '<div>'
    ...
    + '<input value="' + esc(s.marketQuery) + '" ' + T.inp((e) => T.setState({ marketQuery: e.target.value }), 'marketQuery') + ' placeholder="Search markets…" style="background:var(--panel); border:1px solid rgba(var(--ink),.35); border-radius:var(--r-control); padding:9px 12px; ' + M + '; font-size:var(--t-small); color:var(--text); width:230px" />'
    + '<div ' + T.act(() => T.setState({ mPlatform: 'all', mStatus: 'active', mProb: 'all', mLiq: 'all', mVol: 'all', mEnds: 'all', mAge: 'all', mExclude: [], marketCat: 'All', marketQuery: '', mQuick: 'trending', marketSort: 'volume' })) + ' class="hv-bd32" style="font-size:var(--t-small); color:rgba(var(--ink),.6); border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); padding:9px 13px; cursor:pointer">Reset filters</div>'
    + '</div></div>'
    ...
    + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6)">' + mRows.length + ' of ' + T.markets.length + ' markets</div>'
    + mActive.map((c) =>
      '<div ' + T.act(() => T.setState(c.reset)) + ' class="hv-bd30" style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.7); border:1px solid rgba(var(--ink),.14); background:var(--panel-hover); border-radius:var(--r-control); padding:3px 9px; cursor:pointer">' + esc(c.label) + ' ✕</div>'
    ).join('')
    + '</div></div>'
    ...
    + (mRows.length === 0 ? '<div style="padding:60px; text-align:center; ' + M + '; font-size:var(--t-small); color:rgba(var(--ink),.55)">No market matches that filter.</div>' : '')
    + '</div>';
}
...
function tapePulsHtml(prints, buys, sells) {
  ...
  const eintrag = (inhalt) => '<div style="display:flex; align-items:center; gap:6px; ' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.65)">' + inhalt + '</div>';
...
function kategorieFlussHtml(prints) {
  ...
  const zeilen = rows.map((r) => {
    ...
    return '<div style="display:grid; grid-template-columns:104px 1fr 76px 66px; gap:10px; align-items:center; padding:8px 14px; border-bottom:1px solid rgba(var(--ink),.05)">'
      + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.65); white-space:nowrap; overflow:hidden; text-overflow:ellipsis" title="' + esc(r.cat) + '">' + esc(r.cat.toUpperCase()) + '</div>'
      ...
      + '<div style="' + M + '; font-size:var(--t-micro); text-align:right; color:rgba(var(--ink),.55)">' + kaufAnteil + '% buy</div></div>';
  }).join('');
...
export function renderFlow(T) {
  ...
  return '<div>'
    ...
    + '<input value="' + esc(s.tapeQuery) + '" ' + T.inp((e) => T.setState({ tapeQuery: e.target.value }), 'tapeQuery') + ' placeholder="market, wallet, trader…" style="background:var(--panel); border:1px solid rgba(var(--ink),.35); border-radius:var(--r-control); padding:9px 12px; ' + M + '; font-size:var(--t-small); color:var(--text); width:250px" />'
    + '</div></div>'
    ...
    + tapeFiltered.map((t0) => {
      ...
      return '<div ' + (klickbar ? t.act + ' ' : '') + 'class="' + (klickbar ? 'hv-panel' : '') + (neu ? ' tape-in' : '') + '" style="display:grid; grid-template-columns:96px 160px 1fr 110px 84px 90px 110px 96px; align-items:center; padding:12px 24px; border-bottom:1px solid rgba(var(--ink),.06); ' + M + '; font-size:var(--t-small); ' + (klickbar ? 'cursor:pointer; ' : '') + '">'
        ...
        + '<div style="font-size:var(--t-micro); color:rgba(var(--ink),.55)">' + esc(t.category || 'Other') + '</div>'
        + '<div style="' + t.sideStyle + '">' + esc(t.side) + '</div>'
        + '<div style="text-align:right">' + esc(t.price) + '</div>'
        + '<div style="text-align:right">' + t.size + '</div>'
        + '<div style="text-align:right; color:rgba(var(--ink),.6); font-size:var(--t-micro)">' + esc(t.venue) + '</div></div>';
    }).join('')
    + '</div>';
}
...
function crossGateBlock(T) {
  ...
  return '<div style="padding:26px 24px">'
    ...
    + '<div style="font-size:var(--t-body); color:rgba(var(--ink),.6); margin-top:9px; line-height:1.6">No cross-venue pair clears the match gate right now (' + CROSS_GATE_TEXT + '). '
...
export function renderCross(T) {
  ...
  if (!T.crossPairs.length) {
    ...
    if (!hk) {
      ...
      body = leerBlock('NO PAIRS', herkunftSatz(hk, '/api/cross'))
        + (T.neuLaden ? '<div style="padding:0 24px 24px"><div ' + T.act(() => T.neuLaden('cross', 'cross')) + ' class="hv-bd32" style="display:inline-block; ' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.7); border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); padding:6px 12px; cursor:pointer">Try again</div></div>' : '');
    } else {
  ...
  const stepGroup = (label, valueLabel, onDown, onUp) =>
    ...
    + '<div ' + T.act(onDown) + ' class="hv-bd35w" style="width:26px; height:30px; flex:none; border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); display:flex; align-items:center; justify-content:center; ' + M + '; font-size:var(--t-body); color:rgba(var(--ink),.7); cursor:pointer">−</div>'
    + '<div style="flex:1; background:var(--panel); border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); padding:6px 8px; ' + M + '; font-size:var(--t-small); text-align:center">' + esc(valueLabel) + '</div>'
    + '<div ' + T.act(onUp) + ' class="hv-bd35w" style="width:26px; height:30px; flex:none; border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); display:flex; align-items:center; justify-content:center; ' + M + '; font-size:var(--t-body); color:rgba(var(--ink),.7); cursor:pointer">+</div>'
    + '</div></div>';
  ...
  return '<div>'
    ...
    + '<div style="display:flex; align-items:center; gap:10px">'
    + asOfLine(cl.as_of)
    + '<input value="' + esc(s.crossQuery) + '" ' + T.inp((e) => T.setState({ crossQuery: e.target.value }), 'crossQuery') + ' placeholder="bitcoin, fed, election…" style="background:var(--panel); border:1px solid rgba(var(--ink),.35); border-radius:var(--r-control); padding:9px 12px; ' + M + '; font-size:var(--t-small); color:var(--text); width:230px" />'
    + '<div ' + T.act(() => T.setState({ crossQuery: '', crossSim: 0.5, crossMaxPairs: 50, crossMinGap: 0, crossLower: 'any', crossPmVol: 0, crossKsVol: 0, crossMinPrice: 0, crossMaxPrice: 100 })) + ' class="hv-bd32" style="font-size:var(--t-small); color:rgba(var(--ink),.6); border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); padding:9px 13px; cursor:pointer">Reset filters</div>'
    + '</div></div>'
    ...
    + '<div style="display:flex; align-items:center; gap:8px; margin-top:12px; flex-wrap:wrap">'
    + activeChips.map((c) => '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.7); border:1px solid rgba(var(--ink),.14); background:var(--panel-hover); border-radius:var(--r-control); padding:3px 9px">' + esc(c) + '</div>').join('')
    + '</div></div>'

    + '<div style="display:grid; grid-template-columns:repeat(4,1fr); border-bottom:1px solid rgba(var(--ink),.09)">'
    + '<div style="padding:14px 24px; border-right:1px solid rgba(var(--ink),.09)"><div style="' + HEAD_CELL + '">PAIRS SHOWN</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px">' + cRows.length + '</div></div>'
    + '<div style="padding:14px 24px; border-right:1px solid rgba(var(--ink),.09)"><div style="' + HEAD_CELL + '">LARGEST GAP</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px; color:var(--warn)">' + (gaps.length ? gaps[gaps.length - 1] + '¢' : '—') + '</div></div>'
    + '<div style="padding:14px 24px; border-right:1px solid rgba(var(--ink),.09)"><div style="' + HEAD_CELL + '">MEDIAN SIMILARITY</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px">' + (medianSim ? medianSim.toFixed(2) : '—') + '</div></div>'
    + '<div style="padding:14px 24px"><div style="' + HEAD_CELL + '">POSITIVE NET OF FEES</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px; color:' + (netPositive ? 'var(--pos)' : 'rgba(var(--ink),.6)') + '">' + netPositive + ' of ' + netKnown + '</div></div>'
    + '</div>'
    ...
    + cRows.map((c) => {
      ...
      return '<div style="display:grid; grid-template-columns:1fr 104px 104px 84px 108px 124px 112px; align-items:center; padding:13px 24px; border-bottom:1px solid rgba(var(--ink),.06)">'
        ...
        + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); margin-top:3px">' + esc(c.cat) + ' · similarity ' + c.sim.toFixed(2) + '</div></div>'
        ...
        + '<div style="' + M + '; font-size:var(--t-small); text-align:right; color:rgba(var(--ink),.6)">' + volume(c.pmVolUsd, 'Polymarket') + '</div>'
        + '<div style="' + M + '; font-size:var(--t-small); text-align:right; color:rgba(var(--ink),.6)" title="Kalshi reports volume as a contract count; one contract settles for $1, so this is not a dollar figure">' + volume(c.ksVolContracts, 'Kalshi') + '</div>'
    ...
    + (cRows.length === 0 ? '<div style="padding:60px; text-align:center; ' + M + '; font-size:var(--t-small); color:rgba(var(--ink),.55)">No pair passes the local filters; loosen a stepper above.</div>' : '')
    + '</div>';
}
...
export function renderResolved(T) {
  ...
  return '<div>'
    ...
    + '<input value="' + esc(s.resQuery) + '" ' + T.inp((e) => T.setState({ resQuery: e.target.value }), 'resQuery') + ' placeholder="Search resolved markets…" style="background:var(--panel); border:1px solid rgba(var(--ink),.35); border-radius:var(--r-control); padding:9px 12px; ' + M + '; font-size:var(--t-small); color:var(--text); width:250px" />'
    + '</div>'
    + '<div style="font-size:var(--t-body); color:rgba(var(--ink),.55); margin-top:10px; max-width:700px">The last price before settlement next to the answer. The gap between the two is what the crowd got wrong.</div>'
    + '<div style="display:grid; grid-template-columns:repeat(4, minmax(0,1fr)); gap:14px 18px; margin-top:14px">'
    ...
    + resRows.map((r) => {
      ...
      const errStyle = M + '; font-size:var(--t-body); text-align:right; color:' + (r.err >= 50 ? 'var(--neg)' : r.err >= 25 ? 'var(--warn)' : 'rgba(var(--ink),.6)');
      return '<div style="display:grid; grid-template-columns:1fr 110px 118px 128px 110px 120px; align-items:center; padding:13px 24px; border-bottom:1px solid rgba(var(--ink),.06)">'
        ...
        + '<div style="' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); margin-top:3px">' + esc(r.meta) + '</div></div>'
        ...
        + '<div style="' + M + '; font-size:var(--t-small); text-align:right; color:rgba(var(--ink),.6)">' + esc(r.vol) + '</div>'
        + '<div style="' + M + '; font-size:var(--t-small); text-align:right; color:rgba(var(--ink),.6)">' + esc(r.when) + '</div></div>';
    }).join('')
    + '</div>';
}
