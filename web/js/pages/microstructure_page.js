...
const MUTED = 'color:rgba(234,230,220,.55)';
const HR = 'border-top:1px solid rgba(234,230,220,.07); margin-top:20px; padding-top:18px';

const VERDIKT_FARBE = { ja: '#D9A648', nein: '#DE8A7A', offen: '#DE7E36', kontrolle: '#86BDB2' };
...
const DEUTUNG_FARBE = { lesart: '#D9A648', gegenlesart: '#6E9BC8', grenze: '#98978E' };

function abschnitt(titel, inhalt, zusatz) {
  if (!inhalt) return '';
  return '<div style="' + HR + '">'
    + '<h2 style="' + M + '; font-size:11px; letter-spacing:.15em; color:#6E9BC8; margin:0 0 12px; font-weight:400">'
    + esc(titel) + (zusatz ? ' <span style="color:rgba(234,230,220,.5)">' + esc(zusatz) + '</span>' : '')
    + '</h2>' + inhalt + '</div>';
}
...
function analyseBlock(analyse) {
  ...
  return '<div style="display:grid; grid-template-columns:repeat(auto-fit,minmax(280px,1fr)); gap:1px; '
    + 'background:rgba(234,230,220,.07); border:1px solid rgba(234,230,220,.07); border-radius:6px; overflow:hidden">'
    + analyse.map((a) =>
      '<div style="background:#131311; padding:14px 16px">'
      + '<div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">'
      + esc(a.titel) + '</div>'
      + '<div style="font-size:12.5px; color:rgba(234,230,220,.78); margin-top:7px; line-height:1.6">'
      + esc(a.text) + '</div></div>'
    ).join('')
    + '</div>';
}
...
function deutungBlock(interpretation) {
  ...
  return interpretation.map((i) => {
    const farbe = DEUTUNG_FARBE[i.art] || '#98978E';
    return '<div style="border-left:2px solid ' + farbe + '66; padding:2px 0 2px 14px; margin-bottom:14px">'
      ...
      + esc(i.titel) + '</div>'
      + '<div style="font-size:13px; color:rgba(234,230,220,.72); margin-top:6px; line-height:1.65; max-width:720px">'
      + esc(i.text) + '</div></div>';
  }).join('');
}
...
function zahlenBlock(zahlen) {
  ...
  return '<div style="' + CARD + '; padding:6px 0">'
    + zahlen.map((z) =>
      '<div style="display:grid; grid-template-columns:1fr auto; gap:14px; align-items:baseline; padding:9px 16px; border-bottom:1px solid rgba(234,230,220,.05)">'
      + '<div><div style="font-size:12.5px; color:rgba(234,230,220,.78)">' + esc(z.label) + '</div>'
      + (z.hinweis ? '<div style="font-size:11px; ' + MUTED + '; margin-top:3px; line-height:1.45">' + esc(z.hinweis) + '</div>' : '')
      + '</div>'
      + '<div style="' + M + '; font-size:14px; color:#EAE6DC; white-space:nowrap">' + esc(fmtZahl(z.wert))
      + (z.einheit ? ' <span style="font-size:10.5px; color:rgba(234,230,220,.6)">' + esc(z.einheit) + '</span>' : '')
      + '</div></div>'
    ).join('')
    + '</div>';
}
...
function detailBlock(details, id) {
  ...
  const kopf = details.spalten.map((c, i) =>
    '<th style="' + M + '; font-size:10.5px; letter-spacing:.1em; color:rgba(234,230,220,.6); '
    + 'text-align:' + (i === 0 ? 'left' : 'right') + '; padding:8px 12px; white-space:nowrap; '
    + 'border-bottom:1px solid rgba(234,230,220,.09)">' + esc(c) + '</th>').join('');
  const koerper = details.zeilen.map((zeile) =>
    '<tr>' + zeile.map((z, i) =>
      '<td style="' + (i === 0 ? 'font-size:12px' : M + '; font-size:11.5px')
      + '; color:rgba(234,230,220,' + (i === 0 ? '.75' : '.62') + '); '
      + 'text-align:' + (i === 0 ? 'left' : 'right') + '; padding:7px 12px; white-space:nowrap; '
      + 'border-bottom:1px solid rgba(234,230,220,.04)">' + esc(String(z)) + '</td>').join('')
    + '</tr>').join('');
  ...
  return '<div style="margin-top:16px">'
    + '<div style="' + M + '; font-size:10.5px; letter-spacing:.13em; color:rgba(234,230,220,.55)">'
    + esc(details.titel) + ' · ' + details.zeilen.length + ' rows</div>'
    + '<div style="overflow-x:auto; border:1px solid rgba(234,230,220,.07); border-radius:4px; margin-top:8px">'
    + '<table style="width:100%; border-collapse:collapse"><thead><tr>' + kopf + '</tr></thead>'
    + '<tbody>' + koerper + '</tbody></table></div>'
    + (details.hinweis
      ? '<div style="font-size:11.5px; ' + MUTED + '; padding:9px 2px 0; line-height:1.55">' + esc(details.hinweis) + '</div>'
      : '')
    + '</div>';
}
...
function basisZeile(basis) {
  ...
  return '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6)">DATA · ' + esc(teile.join(' · ')) + '</div>';
}
...
function quelleLinks(s) {
  const link = (pfad, text) =>
    ...
    + '" target="_blank" rel="noopener" style="' + M + '; font-size:10.5px; color:#6E9BC8; text-decoration:none; '
    + 'border:1px solid rgba(110,155,200,.35); border-radius:4px; padding:5px 9px">' + esc(text) + ' ↗</a>';
...
function methodeBlock(s) {
  ...
  const inhalt = abschnitt('WHAT WAS ANALYSED', analyseBlock(s.analyse))
    + abschnitt('WHAT THE NUMBERS SAY',
      '<div style="font-size:13.5px; color:rgba(234,230,220,.75); line-height:1.7; max-width:760px">'
      + esc(s.einfach) + '</div>')
  ...
  return '<details data-key="method:' + esc(String(s.id || '')) + '" style="' + CARD + '; margin-top:14px; overflow:hidden">'
    + '<summary style="' + M + '; font-size:10.5px; letter-spacing:.1em; color:rgba(234,230,220,.6); '
    + 'padding:13px 16px; cursor:pointer; list-style:none">▸ METHOD, HOW TO READ IT'
    + (roh ? ' &amp; THE RAW ROWS' : '')
    + ' <span style="color:rgba(234,230,220,.55)">· what was analysed, what else fits the numbers'
    + (roh ? ', every row behind them' : '') + '</span></summary>'
    + '<div style="padding:0 18px 18px; border-top:1px solid rgba(234,230,220,.07)">' + inhalt + '</div>'
    + '</details>';
}
...
function verdiktZeile(studien) {
  ...
  return '<div style="' + M + '; font-size:11.5px; color:rgba(234,230,220,.55); margin-top:12px">'
    ...
    + ' <span style="color:rgba(234,230,220,.55)">· ' + z.gesamt + ' studies</span></div>';
}
...
function sprungliste(studien) {
  ...
  return '<div style="display:flex; gap:6px; flex-wrap:wrap; margin-top:12px">'
    + studien.map((s, i) => {
      const farbe = VERDIKT_FARBE[s.verdikt_art] || '#98978E';
      return '<a href="#' + esc(studieAnker(s, i)) + '" style="' + M + '; font-size:11px; color:rgba(234,230,220,.7); text-decoration:none; '
        + 'border:1px solid rgba(234,230,220,.14); border-left:2px solid ' + farbe + '; border-radius:4px; padding:4px 8px; white-space:nowrap">'
        + esc(kurzLabel(s, i)) + '</a>';
    }).join('')
    + '</div>';
}
...
function studieKarte(s, i) {
  ...
  return '<div id="' + esc(studieAnker(s, i)) + '" style="' + CARD + '; padding:22px 24px; margin-bottom:18px; scroll-margin-top:16px">'
    ...
    + '<div style="' + M + '; font-size:11px; letter-spacing:.16em; color:rgba(234,230,220,.55)">STUDY '
...
function kopf(payload) {
  ...
  const kachel = (wert, text, farbe) =>
    ...
    + '<div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6); margin-top:5px">'
    + esc(text) + '</div></div>';
  ...
  return '<div style="padding:2px 0 0">'
    ...
    + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); border:1px solid rgba(234,230,220,.14); '
    ...
    + kachel(z.gesamt || 0, 'STUDIES', '#EAE6DC')
    + kachel(z.nein || 0, 'REFUTED', '#DE8A7A')
    + kachel(z.ja || 0, 'CONFIRMED', '#D9A648')
    + kachel(z.offen || 0, 'NOT IDENTIFIED', '#DE7E36')
    + (z.kontrolle ? kachel(z.kontrolle, 'CONTROL', '#86BDB2') : '')
    + '</div>'
    + (payload.hinweis
      ? '<div style="font-size:12px; color:rgba(234,230,220,.6); margin-top:14px; line-height:1.55; max-width:760px; '
        + 'border-left:2px solid rgba(234,230,220,.14); padding-left:12px">' + esc(payload.hinweis) + '</div>'
      : '')
...
export function renderMicrostructure(payload) {
  ...
  const fehlend = Array.isArray(payload.fehlend) && payload.fehlend.length
    ? '<div style="' + M + '; font-size:11px; color:#DE7E36; margin-bottom:14px">'
