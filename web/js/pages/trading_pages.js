...
function ohneBacktestHtml(s) {
  ...
  return '<div style="background:var(--panel); border:1px solid var(--line-2); border-radius:var(--r-panel); padding:22px 24px; margin-top:14px; max-width:760px">'
    + '<div style="font-size:var(--t-lead); font-weight:600; color:' + (running ? 'var(--warn)' : err ? 'var(--neg-soft)' : 'var(--text)') + '">' + kopf + '</div>'
    + '<div style="font-size:var(--t-body); color:var(--ink-4); margin-top:10px; line-height:1.65">' + text + '</div>'
    + '<div style="font-size:var(--t-small); color:var(--ink-3); margin-top:12px; line-height:1.6">'
...
function laufStatusHtml(s, hatErgebnis) {
  ...
  if (s.btRun === 'error' && s.btError) {
    return '<div style="margin-top:8px; ' + M + '; font-size:var(--t-micro); color:var(--neg-soft)">' + esc(s.btError) + (hatErgebnis ? ' <span style="color:var(--ink-3)">— the last result is kept below</span>' : '') + '</div>';
  }
...
export function renderBacktester(T) {
  ...
  let tabBody = '';
  if (s.btTab === 'log') {
    tabBody = '<div style="border:1px solid var(--line-2); border-radius:var(--r-panel); margin-top:12px; overflow:hidden">'
      + '<div style="display:grid; grid-template-columns:88px 74px 84px 1fr 60px 84px 78px 74px 84px 88px; gap:10px; padding:9px 16px; background:var(--panel); border-bottom:1px solid var(--line-2); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:var(--ink-3)">'
      ...
      + logRows.map((l) =>
        '<div style="display:grid; grid-template-columns:88px 74px 84px 1fr 60px 84px 78px 74px 84px 88px; gap:10px; align-items:center; padding:10px 16px; border-bottom:1px solid var(--line-3); ' + M + '; font-size:var(--t-small)">'
    ...
    tabBody = '<div style="border:1px solid var(--line-2); border-radius:var(--r-panel); margin-top:12px; overflow:hidden">'
      + '<div style="display:grid; grid-template-columns:1fr 62px 78px 78px 78px 88px 88px 100px; gap:10px; padding:9px 16px; background:var(--panel); border-bottom:1px solid var(--line-2); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:var(--ink-3)">'
    ...
    tabBody = '<div style="background:var(--panel); border:1px solid var(--line-2); border-radius:var(--r-panel); margin-top:12px; padding:16px 18px">'
      + '<div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:var(--ink-4); margin-bottom:12px">DRAWDOWN FROM THE RUNNING PEAK</div>'
      ...
      + '<div style="display:flex; justify-content:space-between; ' + M + '; font-size:var(--t-micro); color:var(--ink-3); margin-top:8px">'
  ...
  return '<div>'
    ...
    + caveatZeile('backtest_modeled', {
      ...
      stil: 'font-size:var(--t-body); color:var(--ink-4); margin-top:9px; max-width:680px; line-height:1.5'
    }) + '</div>'
    ...
    + '<div style="border-right:1px solid var(--line-2); padding:18px 20px; display:flex; flex-direction:column; gap:22px">'

    + '<div><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.16em; color:var(--accent); margin-bottom:9px">01 · WALLET TO REPLAY</div>'
    + '<input value="' + esc(s.btWallet) + '" ' + T.inp((e) => { T.state.btWallet = e.target.value; if (T.liveData.backtest) T.state.btDirty = true; }, 'btWallet') + ' aria-label="Wallet address to replay" placeholder="0x…" style="width:100%; box-sizing:border-box; background:var(--panel); border:1px solid var(--line-edge); border-radius:var(--r-control); padding:10px 12px; ' + M + '; font-size:var(--t-small); color:var(--text)" />'
    + '<div style="font-size:var(--t-small); color:var(--ink-3); margin-top:7px; line-height:1.5">Any public Polymarket address — the default is one with a long public trade history, chosen so a first run has something to replay. Take one from the leaderboard to compare.</div></div>'
    ...
    + (s.btSizing === 'fixed' || s.btSizing === 'pct'
      ? '<div style="margin-top:12px"><div style="' + LABEL_BLOCK + '">AUTO-FIT STAKE TO THE WALLET\'S PACE</div>'
        ...
        + '</div>'
        + '<div style="font-size:var(--t-small); color:var(--ink-3); margin-top:7px; line-height:1.5">'
        + (s.btAutoFit
    ...
    + '</div>'
    + '<div style="font-size:var(--t-small); color:var(--ink-3); margin-top:7px; line-height:1.5">New copies are skipped while open copies already tie up that share of the bankroll. Sells free the room up again.</div></div>'
    ...
    + '</div>'
    + '<div style="font-size:var(--t-small); color:var(--ink-3); margin-top:7px; line-height:1.5">' + (s.btStrategy === 'copy' ? 'Mirror every trade the wallet makes, at the same price.' : 'Fade takes the opposite side of every trade the wallet makes.') + '</div></div>'
    ...
    + (s.btRun === 'running'
      ? '<div style="font-size:var(--t-body); font-weight:600; text-align:center; color:var(--ink-4); border:1px solid var(--line-1); border-radius:var(--r-control); padding:12px; cursor:default">running…</div>'
    ...
    + (filteredN > 0 ? '<div style="border:1px solid var(--line-2); border-radius:var(--r-panel); padding:11px 15px; margin-top:12px; font-size:var(--t-small); color:var(--ink-3); line-height:1.5">'
    ...
    + '<div style="border:1px solid var(--line-2); border-radius:var(--r-panel); margin-top:16px; overflow:hidden">'
    ...
    + (s.sizingSimOpen && bestVariant ?
      ...
      + '<div style="display:grid; grid-template-columns:1fr 110px 96px 96px 96px 88px 88px; gap:10px; padding:9px 14px; background:var(--panel); border-bottom:1px solid var(--line-2); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:var(--ink-3)">'
      ...
      + simVariants.map((v, i) =>
        '<div style="display:grid; grid-template-columns:1fr 110px 96px 96px 96px 88px 88px; gap:10px; align-items:center; padding:10px 14px; border-bottom:1px solid var(--line-3); background:' + (i === 0 ? 'rgba(var(--warn-rgb),.07)' : 'transparent') + '">'
        ...
        + '<div style="text-align:right; ' + M + '; font-size:var(--t-small); color:var(--ink-3)">' + (v.closed ? Math.round(v.wr) + '% <span style="font-size:var(--t-micro); color:var(--ink-4)">n ' + num(v.closed) + '</span>' : '—') + '</div>'
...
function ohnePapierDatenHtml(titel, kicker, grund) {
  return '<div>'
    ...
    + '<div style="background:var(--panel); border:1px solid var(--line-2); border-radius:var(--r-panel); padding:22px 24px; max-width:760px">'
    + '<div style="font-size:var(--t-lead); font-weight:600">Nothing to show</div>'
    + '<div style="font-size:var(--t-body); color:var(--ink-4); margin-top:10px; line-height:1.65">' + grund + '</div>'
    + '<div style="font-size:var(--t-small); color:var(--ink-3); margin-top:12px; line-height:1.6">'
...
export function renderPortfolio(T) {
  ...
  if (s.portTab === 'positions') {
    ...
    body = '<div>'
      ...
      + '<div style="display:grid; grid-template-columns:1fr 76px 92px 92px 100px 100px; gap:10px; padding:9px 16px; background:var(--panel); border-bottom:1px solid var(--line-2); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:var(--ink-3)">'
    ...
    body = '<div style="padding:16px 24px">'
      ...
      + '<div style="background:var(--panel); border:1px solid var(--line-2); border-radius:var(--r-panel); padding:14px 16px"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.13em; color:var(--ink-3)">COPY EQUITY</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px">$' + num((+kp.equity).toFixed(2)) + '</div></div>'
      + '<div style="background:var(--panel); border:1px solid var(--line-2); border-radius:var(--r-panel); padding:14px 16px"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.13em; color:var(--ink-3)">RESEARCH CASH</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px">$' + (+kp.cash).toFixed(2) + '</div></div>'
      + '<div style="background:var(--panel); border:1px solid var(--line-2); border-radius:var(--r-panel); padding:14px 16px"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.13em; color:var(--ink-3)">EVERYTHING TOGETHER</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px; color:var(--accent)">$' + num(((+kp.equity) + (+kp.cash)).toFixed(2)) + '</div></div>'
      + '</div>'
      + '<div style="background:var(--panel); border:1px solid var(--line-2); border-radius:var(--r-panel); padding:16px 18px; margin-top:14px">'
      + '<div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:var(--ink-4); margin-bottom:12px">COMBINED EQUITY</div>'
    ...
    body = '<div style="padding:16px 24px; display:grid; grid-template-columns:1fr 1fr; gap:20px">'
      + '<div><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:var(--ink-4); margin-bottom:14px">BY CATEGORY</div>'
      + '<div style="display:flex; flex-direction:column; gap:14px">'
      + (alloc.length ? '' : leerZeile(keinBuch))
      + alloc.map((a) =>
        '<div><div style="display:flex; justify-content:space-between; ' + M + '; font-size:var(--t-small); margin-bottom:6px"><span style="color:var(--ink-3)">' + a.label + '</span><span>' + a.value + '</span></div>'
      ...
      + '</div></div>'
      + '<div><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:var(--ink-4); margin-bottom:14px">CONCENTRATION</div>'
    ...
    body = '<div style="border:1px solid var(--line-2); border-radius:var(--r-panel); margin:14px 24px; overflow:hidden">'
      + '<div style="display:grid; grid-template-columns:110px 1fr 78px 92px 92px 100px; gap:10px; padding:9px 16px; background:var(--panel); border-bottom:1px solid var(--line-2); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:var(--ink-3)">'
  ...
  return '<div>'
    ...
    + '<div style="padding:16px 20px; border-right:1px solid var(--line-2)"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:var(--ink-3)">VALUE NOW</div><div style="' + M + '; font-size:var(--t-hero); margin-top:8px">$' + num((+kp.equity).toFixed(2)) + '</div></div>'
    ...
    + '<div style="padding:16px 20px; border-right:1px solid var(--line-2)"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:var(--ink-3)">OPEN POSITIONS</div><div style="' + M + '; font-size:var(--t-hero); margin-top:8px">' + (kp.open_positions != null ? num(kp.open_positions) : '—') + '</div></div>'
    + '<div style="padding:16px 20px; border-right:1px solid var(--line-2)"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:var(--ink-3)">UNREALISED</div><div style="' + M + '; font-size:var(--t-hero); margin-top:8px; color:' + (kp.unrealized == null ? 'var(--ink-3)' : +kp.unrealized >= 0 ? 'var(--pos)' : 'var(--neg)') + '">' + (kp.unrealized != null ? (+kp.unrealized >= 0 ? '+' : '-') + '$' + Math.abs(+kp.unrealized).toFixed(2) : '—') + '</div></div>'
    + '<div style="padding:16px 20px"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:var(--ink-3)">CASH FREE</div><div style="' + M + '; font-size:var(--t-hero); margin-top:8px">' + (kp.cash != null ? '$' + (+kp.cash).toFixed(2) : '—') + '</div></div>'
    + '</div>'
