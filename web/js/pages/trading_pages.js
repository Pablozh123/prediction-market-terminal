...
const LBL95 = M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.6); margin-bottom:7px';
...
function ohneBacktestHtml(s) {
  ...
  return '<div style="background:var(--panel); border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); padding:22px 24px; margin-top:14px; max-width:760px">'
    + '<div style="font-size:var(--t-lead); font-weight:600; color:' + (running ? 'var(--warn)' : err ? 'var(--neg-soft)' : 'var(--text)') + '">' + kopf + '</div>'
    + '<div style="font-size:var(--t-body); color:rgba(var(--ink),.55); margin-top:10px; line-height:1.65">' + text + '</div>'
    + '<div style="font-size:var(--t-small); color:rgba(var(--ink),.6); margin-top:12px; line-height:1.6">'
...
function laufStatusHtml(s, hatErgebnis) {
  if (s.btRun === 'running') {
    return '<div style="display:flex; align-items:center; gap:8px; margin-top:8px; ' + M + '; font-size:var(--t-micro); color:var(--warn)">'
  ...
  if (s.btRun === 'error' && s.btError === 'rate-limited') {
    return '<div style="margin-top:8px; ' + M + '; font-size:var(--t-micro); color:var(--warn)">rate-limited, retry in ' + (s.btRetryIn > 0 ? s.btRetryIn + ' s' : 'a moment')
      + (hatErgebnis ? ' <span style="color:rgba(var(--ink),.6)">— the last result is kept below</span>' : '') + '</div>';
  }
  if (s.btRun === 'error' && s.btError) {
    return '<div style="margin-top:8px; ' + M + '; font-size:var(--t-micro); color:var(--neg-soft)">' + esc(s.btError) + (hatErgebnis ? ' <span style="color:rgba(var(--ink),.6)">— the last result is kept below</span>' : '') + '</div>';
  }
  if (hatErgebnis && s.btDirty) {
    return '<div style="margin-top:8px; ' + M + '; font-size:var(--t-micro); color:var(--ink-4)">settings changed since this run — press RUN to refresh</div>';
  }
  return '';
}
...
export function renderBacktester(T) {
  ...
  const stepRow = (label, valueLabel, down, up) =>
    ...
    + '<div ' + T.act(bt(T, down)) + ' style="width:28px; height:32px; flex:none; border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); display:flex; align-items:center; justify-content:center; ' + M + '; font-size:var(--t-body); color:rgba(var(--ink),.7); cursor:pointer">−</div>'
    ...
    + '<div ' + T.act(bt(T, up)) + ' style="width:28px; height:32px; flex:none; border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); display:flex; align-items:center; justify-content:center; ' + M + '; font-size:var(--t-body); color:rgba(var(--ink),.7); cursor:pointer">+</div></div></div>';
  ...
  let tabBody = '';
  if (s.btTab === 'log') {
    tabBody = '<div style="border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); margin-top:12px; overflow:hidden">'
      + '<div style="display:grid; grid-template-columns:88px 74px 84px 1fr 60px 84px 78px 74px 84px 88px; gap:10px; padding:9px 16px; background:var(--panel); border-bottom:1px solid rgba(var(--ink),.09); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:rgba(var(--ink),.6)">'
      ...
      + logRows.map((l) =>
        '<div style="display:grid; grid-template-columns:88px 74px 84px 1fr 60px 84px 78px 74px 84px 88px; gap:10px; align-items:center; padding:10px 16px; border-bottom:1px solid rgba(var(--ink),.06); ' + M + '; font-size:var(--t-small)">'
        ...
        + '<div><span style="' + M + '; font-size:var(--t-micro); letter-spacing:.08em; border-radius:var(--r-control); padding:2px 6px; ' + (l.status === 'skipped' ? 'color:var(--warn); border:1px solid rgba(var(--warn-rgb),.35)' : 'color:rgba(var(--ink),.7); border:1px solid rgba(var(--ink),.18)') + '">' + esc(l.status) + '</span></div>'
    ...
    tabBody = '<div style="border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); margin-top:12px; overflow:hidden">'
      + '<div style="display:grid; grid-template-columns:1fr 62px 78px 78px 78px 88px 88px 100px; gap:10px; padding:9px 16px; background:var(--panel); border-bottom:1px solid rgba(var(--ink),.09); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:rgba(var(--ink),.6)">'
      ...
      + openRows.map((o) =>
        '<div style="display:grid; grid-template-columns:1fr 62px 78px 78px 78px 88px 88px 100px; gap:10px; align-items:center; padding:10px 16px; border-bottom:1px solid rgba(var(--ink),.06); ' + M + '; font-size:var(--t-small)">'
    ...
    tabBody = '<div style="background:var(--panel); border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); margin-top:12px; padding:16px 18px">'
      + '<div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.55); margin-bottom:12px">DRAWDOWN FROM THE RUNNING PEAK</div>'
      ...
      + '<div style="display:flex; justify-content:space-between; ' + M + '; font-size:var(--t-micro); color:rgba(var(--ink),.6); margin-top:8px">'
  ...
  return '<div>'
    ...
    + '<h1 style="font-size:var(--t-head); line-height:1.25; margin:6px 0 0; font-weight:600; letter-spacing:-0.01em">Replay a wallet with your own sizing</h1>'
    ...
    + caveatZeile('backtest_modeled', {
      ...
      stil: 'font-size:var(--t-body); color:rgba(var(--ink),.55); margin-top:9px; max-width:680px; line-height:1.5'
    }) + '</div>'
    + '<div style="' + M + '; font-size:var(--t-micro); color:var(--on-accent); background:var(--accent); border-radius:var(--r-control); padding:5px 10px">POLYMARKET</div>'
    + '</div></div>'
    ...
    + '<div><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.16em; color:var(--accent); margin-bottom:9px">01 · WALLET TO REPLAY</div>'
    + '<input value="' + esc(s.btWallet) + '" ' + T.inp((e) => { T.state.btWallet = e.target.value; if (T.liveData.backtest) T.state.btDirty = true; }, 'btWallet') + ' aria-label="Wallet address to replay" placeholder="0x…" style="width:100%; box-sizing:border-box; background:var(--panel); border:1px solid rgba(var(--ink),.35); border-radius:var(--r-control); padding:10px 12px; ' + M + '; font-size:var(--t-small); color:var(--text)" />'
    + '<div style="font-size:var(--t-small); color:rgba(var(--ink),.6); margin-top:7px; line-height:1.5">Any public Polymarket address — the default is one with a long public trade history, chosen so a first run has something to replay. Take one from the leaderboard to compare.</div></div>'

    + '<div><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.16em; color:var(--accent); margin-bottom:9px">02 · STAKE PER COPY</div>'
    + '<div style="display:grid; grid-template-columns:1fr 1fr; gap:6px">'
    + Object.keys(SIZING).map((k) =>
      '<div ' + T.act(bt(T, { btSizing: k })) + ' style="font-size:var(--t-small); text-align:center; border-radius:var(--r-control); padding:8px 6px; cursor:pointer; ' + (s.btSizing === k ? 'color:var(--on-accent); background:var(--accent); font-weight:600' : 'color:rgba(var(--ink),.6); border:1px solid rgba(var(--ink),.16)') + '">' + SIZING[k] + '</div>'
    ).join('')
    + '</div>'
    + '<div style="margin-top:12px"><div style="' + LBL95 + '">' + stakeLabel + '</div>'
    + '<div style="display:flex; align-items:center; gap:8px">'
    + '<div ' + T.act(bt(T, stakeDown)) + ' class="hv-bd35w" style="width:32px; height:34px; flex:none; border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); display:flex; align-items:center; justify-content:center; ' + M + '; font-size:var(--t-lead); color:rgba(var(--ink),.7); cursor:pointer">−</div>'
    + '<div style="flex:1; background:var(--panel); border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); padding:8px 12px; ' + M + '; font-size:var(--t-body); text-align:center">' + esc(stakeValue) + '</div>'
    + '<div ' + T.act(bt(T, stakeUp)) + ' class="hv-bd35w" style="width:32px; height:34px; flex:none; border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); display:flex; align-items:center; justify-content:center; ' + M + '; font-size:var(--t-lead); color:rgba(var(--ink),.7); cursor:pointer">+</div></div>'
    + '<div style="font-size:var(--t-small); color:rgba(var(--ink),.6); margin-top:7px; line-height:1.5">' + stakeHint + '</div></div>'
    ...
    + (s.btSizing === 'fixed' || s.btSizing === 'pct'
      ? '<div style="margin-top:12px"><div style="' + LBL95 + '">AUTO-FIT STAKE TO THE WALLET\'S PACE</div>'
        ...
        + '</div>'
        + '<div style="font-size:var(--t-small); color:rgba(var(--ink),.6); margin-top:7px; line-height:1.5">'
        + (s.btAutoFit
    ...
    + '</div>'
    + '<div style="font-size:var(--t-small); color:rgba(var(--ink),.6); margin-top:7px; line-height:1.5">New copies are skipped while open copies already tie up that share of the bankroll. Sells free the room up again.</div></div>'

    + '<div><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.16em; color:var(--accent); margin-bottom:9px">03 · TIME WINDOW</div>'
    ...
    + '<div><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.16em; color:var(--accent); margin-bottom:9px">04 · STRATEGY</div>'
    + '<div style="display:flex; gap:7px">'
    + [T.tab('Copy', s.btStrategy === 'copy', bt(T, { btStrategy: 'copy' })), T.tab('Fade', s.btStrategy === 'fade', bt(T, { btStrategy: 'fade' }))].join('')
    + '</div>'
    + '<div style="font-size:var(--t-small); color:rgba(var(--ink),.6); margin-top:7px; line-height:1.5">' + (s.btStrategy === 'copy' ? 'Mirror every trade the wallet makes, at the same price.' : 'Fade takes the opposite side of every trade the wallet makes.') + '</div></div>'

    + '<div style="border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); overflow:hidden">'
    ...
    + (s.advancedOpen ?
      ...
      + (s.btFeeModel === 'flat'
        ? '<div style="display:flex; flex-direction:column; gap:8px">'
          ...
          + '</div></div>'
        : '<div style="font-size:var(--t-small); color:rgba(var(--ink),.6); line-height:1.5">'
    ...
    + (s.btRun === 'running'
      ? '<div style="font-size:var(--t-body); font-weight:600; text-align:center; color:var(--ink-4); border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); padding:12px; cursor:default">running…</div>'
      : s.btRun === 'error' && s.btError === 'rate-limited' && s.btRetryIn > 0
        ? '<div style="font-size:var(--t-body); font-weight:600; text-align:center; color:var(--warn); border:1px solid rgba(var(--warn-rgb),.4); border-radius:var(--r-control); padding:12px; cursor:default">rate-limited · retry in ' + s.btRetryIn + ' s</div>'
        : '<div ' + T.act(() => { T.setState({ btTab: 'log' }); T.runBacktest(); }) + ' class="hv-accentbg" style="font-size:var(--t-body); font-weight:600; text-align:center; color:var(--on-accent); background:var(--accent); border-radius:var(--r-control); padding:12px; cursor:pointer">RUN backtest →</div>')
    + '</div>'

    + '<div style="padding:18px 24px">'
    + '<div style="' + M + '; font-size:var(--t-small); color:rgba(var(--ink),.6)">' + esc(runMeta) + '</div>'
    ...
    + statCards.map((c) =>
      ...
      + '<div style="' + M + '; font-size:var(--t-micro); letter-spacing:.13em; color:rgba(var(--ink),.6)">' + c.label + '</div>'
      + '<div style="' + M + '; font-size:var(--t-head); margin-top:7px; color:' + (c.pos === true ? 'var(--pos)' : c.pos === false ? 'var(--neg)' : 'var(--text)') + '">' + c.value + '</div>'
      + '<div style="' + M + '; font-size:var(--t-micro); margin-top:5px; color:rgba(var(--ink),.6)">' + c.sub + '</div></div>'
    ).join('')
    + '</div>'
    ...
    + (skippedN > copied ? '<div style="border:1px solid rgba(var(--warn-rgb),.3); background:rgba(var(--warn-rgb),.07); border-radius:var(--r-panel); padding:12px 15px; margin-top:12px; font-size:var(--t-small); color:var(--warn); line-height:1.5">More skipped than copied: ' + num(skippedN) + ' of the wallet\'s trades were not mirrored'
    ...
    + (filteredN > 0 ? '<div style="border:1px solid rgba(var(--ink),.12); border-radius:var(--r-panel); padding:11px 15px; margin-top:12px; font-size:var(--t-small); color:rgba(var(--ink),.6); line-height:1.5">'
    ...
    + (autoFit && autoFit.hindsight && autoFit.note
      ? '<div style="border:1px solid rgba(var(--warn-rgb),.3); background:rgba(var(--warn-rgb),.07); border-radius:var(--r-panel); padding:11px 15px; margin-top:12px; font-size:var(--t-small); color:var(--warn); line-height:1.5">'
    ...
    + '<div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.55)">EQUITY CURVE</div>'
    + '<div style="display:flex; gap:16px; ' + M + '; font-size:var(--t-micro); flex-wrap:wrap">'
    ...
    + '<div style="border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); margin-top:16px; overflow:hidden">'
    ...
    + (s.sizingSimOpen && !bestVariant
      ? '<div style="padding:14px 18px; ' + M + '; font-size:var(--t-micro); color:var(--ink-4)">The variants are computed with the run — press RUN with this section open to include them.</div>'
      : '')
    + (s.sizingSimOpen && bestVariant ?
      ...
      + caveatZeile('backtest_modeled', {
        ...
        stil: 'font-size:var(--t-small); color:rgba(var(--ink),.55); line-height:1.5'
      })
      + '<div style="font-size:var(--t-body); margin-top:12px">Highest final equity in this window: <strong style="color:var(--warn)">' + esc(bestVariant.name) + '</strong> → $' + bestVariant.eq.toFixed(0) + ' final equity (' + (bestVariant.roi >= 0 ? '+' : '') + bestVariant.roi.toFixed(1) + '% ROI)</div>'
      + '<div style="border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); margin-top:14px; overflow:hidden">'
      + '<div style="display:grid; grid-template-columns:1fr 110px 96px 96px 96px 88px 88px; gap:10px; padding:9px 14px; background:var(--panel); border-bottom:1px solid rgba(var(--ink),.09); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:rgba(var(--ink),.6)">'
      ...
      + simVariants.map((v, i) =>
        ...
        + '<div style="text-align:right; ' + M + '; font-size:var(--t-small); color:rgba(var(--ink),.6)">' + v.dd.toFixed(1) + '%</div>'
        + '<div style="text-align:right; ' + M + '; font-size:var(--t-small); color:rgba(var(--ink),.6)">' + (v.closed ? Math.round(v.wr) + '% <span style="font-size:var(--t-micro); color:var(--ink-4)">n ' + num(v.closed) + '</span>' : '—') + '</div>'
        + '<div style="text-align:right; ' + M + '; font-size:var(--t-small); color:rgba(var(--ink),.6)">' + num(v.copied) + '</div>'
        + '<div style="text-align:right; ' + M + '; font-size:var(--t-small); color:rgba(var(--ink),.6)">' + num(Math.max(0, v.skipped)) + '</div></div>'
      ).join('')
      + '</div></div>' : '')
    + '</div>'
...
function ohnePapierDatenHtml(titel, kicker, grund) {
  return '<div>'
    ...
    + '<h1 style="font-size:var(--t-head); line-height:1.25; margin:6px 0 0; font-weight:600; letter-spacing:-0.01em">' + titel + '</h1></div>'
    + '<div style="padding:26px 24px">'
    + '<div style="background:var(--panel); border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); padding:22px 24px; max-width:760px">'
    + '<div style="font-size:var(--t-lead); font-weight:600">Nothing to show</div>'
    + '<div style="font-size:var(--t-body); color:rgba(var(--ink),.55); margin-top:10px; line-height:1.65">' + grund + '</div>'
    + '<div style="font-size:var(--t-small); color:rgba(var(--ink),.6); margin-top:12px; line-height:1.6">'
...
export function renderPortfolio(T) {
  ...
  if (s.portTab === 'positions') {
    ...
    body = '<div>'
      ...
      + '<div><div style="' + LBL9 + '">SEARCH</div><input value="' + esc(s.portQuery) + '" ' + T.inp((e) => T.setState({ portQuery: e.target.value }), 'portQuery') + ' placeholder="market or wallet…" style="width:100%; box-sizing:border-box; background:var(--panel); border:1px solid rgba(var(--ink),.35); border-radius:var(--r-control); padding:8px 10px; ' + M + '; font-size:var(--t-small); color:var(--text)" /></div>'
      ...
      + '<div style="display:grid; grid-template-columns:1fr 76px 92px 92px 100px 100px; gap:10px; padding:9px 16px; background:var(--panel); border-bottom:1px solid rgba(var(--ink),.09); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:rgba(var(--ink),.6)">'
      ...
      + rows.map((r) =>
        ...
        + r.map((v, i) => {
          const style = i === 0 ? "font-family:'IBM Plex Sans',sans-serif; font-size:var(--t-small); white-space:nowrap; overflow:hidden; text-overflow:ellipsis" : M + '; font-size:var(--t-small); text-align:right; color:' + (i === 4 ? (v.charAt(0) === '+' ? 'var(--pos)' : 'var(--neg)') : i === 1 ? (v === 'YES' ? 'var(--pos)' : 'var(--info)') : 'rgba(var(--ink),.7)');
    ...
    body = '<div style="padding:16px 24px">'
      ...
      + '<div style="background:var(--panel); border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); padding:14px 16px"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.13em; color:rgba(var(--ink),.6)">COPY EQUITY</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px">$' + num((+kp.equity).toFixed(2)) + '</div></div>'
      + '<div style="background:var(--panel); border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); padding:14px 16px"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.13em; color:rgba(var(--ink),.6)">RESEARCH CASH</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px">$' + (+kp.cash).toFixed(2) + '</div></div>'
      + '<div style="background:var(--panel); border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); padding:14px 16px"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.13em; color:rgba(var(--ink),.6)">EVERYTHING TOGETHER</div><div style="' + M + '; font-size:var(--t-head); margin-top:7px; color:var(--accent)">$' + num(((+kp.equity) + (+kp.cash)).toFixed(2)) + '</div></div>'
      + '</div>'
      + '<div style="background:var(--panel); border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); padding:16px 18px; margin-top:14px">'
      + '<div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.55); margin-bottom:12px">COMBINED EQUITY</div>'
    ...
    body = '<div style="padding:16px 24px; display:grid; grid-template-columns:1fr 1fr; gap:20px">'
      + '<div><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.55); margin-bottom:14px">BY CATEGORY</div>'
      + '<div style="display:flex; flex-direction:column; gap:14px">'
      + (alloc.length ? '' : leerZeile(keinBuch))
      + alloc.map((a) =>
        '<div><div style="display:flex; justify-content:space-between; ' + M + '; font-size:var(--t-small); margin-bottom:6px"><span style="color:rgba(var(--ink),.66)">' + a.label + '</span><span>' + a.value + '</span></div>'
      ...
      + '</div></div>'
      + '<div><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.55); margin-bottom:14px">CONCENTRATION</div>'
      + '<div style="display:flex; flex-direction:column; gap:11px">'
      + (conc
        ? '<div style="display:flex; justify-content:space-between; font-size:var(--t-body)"><span style="color:rgba(var(--ink),.7)">Biggest single position</span><span style="' + M + '">' + conc.biggest + '</span></div>'
        + '<div style="display:flex; justify-content:space-between; font-size:var(--t-body)"><span style="color:rgba(var(--ink),.7)">Top three positions</span><span style="' + M + '">' + conc.top3 + '</span></div>'
        + '<div style="display:flex; justify-content:space-between; font-size:var(--t-body)"><span style="color:rgba(var(--ink),.7)">Cash not deployed</span><span style="' + M + '">' + conc.cash + '</span></div>'
    ...
    body = '<div style="border:1px solid rgba(var(--ink),.09); border-radius:var(--r-panel); margin:14px 24px; overflow:hidden">'
      + '<div style="display:grid; grid-template-columns:110px 1fr 78px 92px 92px 100px; gap:10px; padding:9px 16px; background:var(--panel); border-bottom:1px solid rgba(var(--ink),.09); ' + M + '; font-size:var(--t-micro); letter-spacing:.12em; color:rgba(var(--ink),.6)">'
      ...
      + histRows.map((r) =>
        ...
        + r.map((v, i) => {
          const style = i === 1 ? "font-family:'IBM Plex Sans',sans-serif; font-size:var(--t-small); white-space:nowrap; overflow:hidden; text-overflow:ellipsis" : M + '; font-size:var(--t-small); text-align:' + (i === 0 ? 'left' : 'right') + '; color:' + (i === 5 ? (v.charAt(0) === '+' ? 'var(--pos)' : 'var(--neg)') : i === 2 ? (v === 'YES' ? 'var(--pos)' : 'var(--info)') : 'rgba(var(--ink),.7)');
          return '<div style="' + style + '">' + esc(String(v)) + '</div>';
        }).join('')
        + '</div>'
      ).join('')
  ...
  return '<div>'
    ...
    + '<h1 style="font-size:var(--t-head); line-height:1.25; margin:6px 0 0; font-weight:600; letter-spacing:-0.01em">What you would be holding</h1></div>'
    + '<div style="display:grid; grid-template-columns:repeat(4,1fr); border-bottom:1px solid rgba(var(--ink),.09)">'
    + '<div style="padding:16px 20px; border-right:1px solid rgba(var(--ink),.09)"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.6)">VALUE NOW</div><div style="' + M + '; font-size:var(--t-hero); margin-top:8px">$' + num((+kp.equity).toFixed(2)) + '</div></div>'
    ...
    + '<div style="padding:16px 20px; border-right:1px solid rgba(var(--ink),.09)"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.6)">OPEN POSITIONS</div><div style="' + M + '; font-size:var(--t-hero); margin-top:8px">' + (kp.open_positions != null ? num(kp.open_positions) : '—') + '</div></div>'
    + '<div style="padding:16px 20px; border-right:1px solid rgba(var(--ink),.09)"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.6)">UNREALISED</div><div style="' + M + '; font-size:var(--t-hero); margin-top:8px; color:' + (kp.unrealized == null ? 'rgba(var(--ink),.6)' : +kp.unrealized >= 0 ? 'var(--pos)' : 'var(--neg)') + '">' + (kp.unrealized != null ? (+kp.unrealized >= 0 ? '+' : '-') + '$' + Math.abs(+kp.unrealized).toFixed(2) : '—') + '</div></div>'
    + '<div style="padding:16px 20px"><div style="' + M + '; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.6)">CASH FREE</div><div style="' + M + '; font-size:var(--t-hero); margin-top:8px">' + (kp.cash != null ? '$' + (+kp.cash).toFixed(2) : '—') + '</div></div>'
    + '</div>'
