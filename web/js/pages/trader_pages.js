...
function scorePartsHtml(t) {
  ...
  return '<div style="display:flex; gap:5px; flex-wrap:wrap; margin-top:4px">'
    + parts.map((p) => '<span style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.5); border:1px solid rgba(234,230,220,.1); border-radius:4px; padding:1px 6px; white-space:nowrap">' + esc(p.label) + ' <span style="color:rgba(234,230,220,.8)">' + esc(p.value) + '</span></span>').join('')
    + '</div>';
}
...
export function renderTraders(T) {
  const s = T.state;
  if (!T.traders.length) {
    return '<div>' + seitenKopf('LEADERBOARD', 'Who is actually good at this', '#D9A648')
  ...
  const badge = tCount ? M + '; font-size:11px; color:#0B0B0A; background:#D9A648; border-radius:4px; padding:1px 7px' : 'display:none';
  const chevron = M + '; font-size:16px; color:rgba(234,230,220,.5); transition:transform .18s ease; transform:rotate(' + (s.traderFiltersOpen ? '90deg' : '0deg') + ')';
  ...
  return '<div>'
    + '<div style="padding:20px 24px 14px; border-bottom:1px solid rgba(234,230,220,.09)">'
    + '<div style="display:flex; align-items:flex-end; justify-content:space-between; gap:20px">'
    + '<div><div style="' + M + '; font-size:11px; letter-spacing:.18em; color:#D9A648">LEADERBOARD</div>'
    ...
    + '<input value="' + esc(s.traderQuery) + '" ' + T.inp((e) => T.setState({ traderQuery: e.target.value }), 'traderQuery') + ' placeholder="Search name or wallet…" style="background:#131311; border:1px solid rgba(234,230,220,.35); border-radius:4px; padding:9px 12px; ' + M + '; font-size:12.5px; color:#EAE6DC; width:230px" />'
    + '<div ' + T.act(() => T.setState({ traderQuery: '', tPnl: 'all', tVol: 'all', traderRank: 'pnl' })) + ' class="hv-bd32" style="font-size:12.5px; color:rgba(234,230,220,.6); border:1px solid rgba(234,230,220,.16); border-radius:4px; padding:9px 13px; cursor:pointer">Reset filters</div>'
    + '</div></div>'
    + '<div style="font-size:13px; color:rgba(234,230,220,.55); margin-top:10px; max-width:760px">Ranked from the public Polymarket all-time leaderboard. The smart score is a weighted composite of the components listed under each wallet; win rate and resolved-bet counts are computed per wallet with sample size and confidence interval — open a wallet to see them.</div>'
    ...
    + '<div ' + T.act(() => T.setState({ traderFiltersOpen: !s.traderFiltersOpen })) + ' class="hv-el" style="display:flex; align-items:center; justify-content:space-between; padding:11px 15px; background:#131311; cursor:pointer">'
    ...
    + (s.traderFiltersOpen ?
      ...
      + '</div>' : '')
    + '</div>'
    + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); margin-top:12px">' + traderSorted.length + ' of ' + T.traders.length + ' wallets · all-time' + esc(asOf) + '</div>'
    + '</div>'

    + '<div style="display:grid; grid-template-columns:' + grid + '; padding:10px 24px; border-bottom:1px solid rgba(234,230,220,.09); background:#131311; ' + HEAD_CELL + '">'
    ...
    + traderSorted.map((t, i) => {
      const score = t.score;
      const avatarStyle = 'width:28px; height:28px; flex:none; border-radius:4px; background:#1C232B; border:1px solid rgba(234,230,220,.09); display:flex; align-items:center; justify-content:center; ' + M + '; font-size:12px; color:' + (score != null && score >= 80 ? '#D9A648' : 'rgba(234,230,220,.6)');
      const scoreStyle = M + '; font-size:12.5px; border-radius:4px; padding:3px 9px; ' + (score == null ? 'color:rgba(234,230,220,.6); border:1px solid rgba(234,230,220,.12)' : score >= 80 ? 'color:#0B0B0A; background:#D9A648' : score >= 60 ? 'color:rgba(234,230,220,.8); border:1px solid rgba(234,230,220,.2)' : 'color:#DE7E36; border:1px solid rgba(222,126,54,.35)');
      return '<div ' + T.act(() => T.openWallet(t.name)) + ' class="hv-panel" style="display:grid; grid-template-columns:' + grid + '; align-items:center; padding:13px 24px; border-bottom:1px solid rgba(234,230,220,.06); cursor:pointer">'
        + '<div style="' + M + '; font-size:13px; color:rgba(234,230,220,.6)">' + (i + 1) + '</div>'
        ...
        + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); margin-top:2px">' + esc(t.wallet) + (t.grade ? ' · grade ' + esc(t.grade) : '') + '</div>'
        + scorePartsHtml(t) + '</div></div>'
        + '<div style="' + M + '; font-size:14px; text-align:right; color:' + (t.pnl >= 0 ? '#69B47E' : '#D95F52') + '">' + money(t.pnl) + '</div>'
        + (hatWin ? '<div style="' + M + '; font-size:13px; text-align:right">' + (t.win != null ? Math.round(t.win * 100) + '%' : '—') + '</div>' : '')
        + (hatResolved ? '<div style="' + M + '; font-size:12.5px; text-align:right; color:rgba(234,230,220,.55)">' + (t.resolved != null ? num(t.resolved) : '—') + '</div>' : '')
...
export function renderWhale(T) {
  ...
  if (!T.tape.length) {
    return '<div>' + seitenKopf('WHALE FLOW', 'Who is moving the big money', '#D9A648')
  ...
  const kpi = (label, value, sub, last) =>
    '<div style="padding:14px 20px' + (last ? '' : '; border-right:1px solid rgba(234,230,220,.09)') + '">'
    + '<div style="' + M + '; font-size:11px; letter-spacing:.14em; color:rgba(234,230,220,.6)">' + label + '</div>'
    + '<div style="' + M + '; font-size:24px; margin-top:7px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis">' + value + '</div>'
    + (sub ? '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); margin-top:3px">' + sub + '</div>' : '')
    + '</div>';
  ...
  return '<div>'
    ...
    + '<div style="' + M + '; font-size:11px; letter-spacing:.18em; color:#D9A648">WHALE FLOW</div>'
    ...
    + '<span style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); margin-left:6px">' + (walletCount > SHOW ? 'top ' + SHOW + ' of ' + num(walletCount) + ' wallets' : num(walletCount) + ' wallet' + (walletCount === 1 ? '' : 's')) + ' · ' + num(grouped.length) + ' print' + (grouped.length === 1 ? '' : 's') + ' grouped</span>'
    + '</div></div>'
    + '<div style="display:grid; grid-template-columns:repeat(5,1fr); border-bottom:1px solid rgba(234,230,220,.09)">'
    ...
    + '</div>'
    + '<div style="padding:10px 24px; border-bottom:1px solid rgba(234,230,220,.09); font-size:12px; color:rgba(234,230,220,.55)"><span style="' + M + '; font-size:10.5px; letter-spacing:.14em; color:rgba(234,230,220,.6); margin-right:8px">CONCENTRATION</span>' + esc(konzentrationSatz) + '</div>'
    + '<div style="display:grid; grid-template-columns:' + GRID + '; gap:0 10px; padding:10px 24px; border-bottom:1px solid rgba(234,230,220,.09); background:#131311; ' + HEAD_CELL + '">'
    ...
    + rows.map((w) => {
      ...
      // gedaempftere Farbton der Palette (#DE8A7A) die beiden Staerken, statt
      // Deckkraft wegzunehmen.
      const leanColor = w.lean === 'BUYING' ? '#69B47E' : w.lean === 'SELLING' ? '#D95F52' : w.lean === 'MOSTLY BUYING' ? 'rgba(105,180,126,.75)' : '#DE8A7A';
      ...
      return '<div ' + T.act(() => T.openWallet(w.name, w.wallet !== w.name ? w.wallet : '')) + ' class="hv-panel" style="display:grid; grid-template-columns:' + GRID + '; gap:0 10px; align-items:center; padding:12px 24px; border-bottom:1px solid rgba(234,230,220,.06); cursor:pointer">'
        ...
        + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); margin-top:3px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis" title="' + esc(w.wallet) + '">' + esc(w.wallet) + ' · ' + esc(w.venues) + '</div></div>'
        + '<div style="' + M + '; font-size:13px; text-align:right; color:rgba(234,230,220,.6)">' + w.prints + '</div>'
        + '<div style="' + M + '; font-size:14px; text-align:right">' + money(w.total) + '</div>'
        + '<div style="' + M + '; font-size:13px; text-align:right; color:rgba(234,230,220,.6)">' + money(w.biggest) + '</div>'
        + '<div><div style="' + leanStyle + '">' + esc(w.lean) + '</div>'
        + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); text-align:right; margin-top:2px">' + w.buys + ' buy' + (w.buys === 1 ? '' : 's') + ' · ' + w.sells + ' sell' + (w.sells === 1 ? '' : 's') + '</div></div>'
        + '<div style="' + M + '; font-size:13px; text-align:right; color:rgba(234,230,220,.6)">' + w.marketCount + '</div>'
        ...
        + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); margin-top:2px">' + money(w.topMarket.dollar) + ' · ' + topMarketShare + '% of this wallet</div></div>'
        + '<div style="text-align:right"><div style="' + M + '; font-size:11.5px; color:rgba(234,230,220,.75)">' + esc(w.cat) + '</div>'
        + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); margin-top:2px">' + esc(w.catShare) + ' prints</div></div>'
        + '<div style="' + M + '; font-size:11px; text-align:right; color:rgba(234,230,220,.5)">' + esc(w.lastAgo) + '</div></div>';
    }).join('')
    + '</div>';
}
...
const CHIP = M + '; font-size:10.5px; color:rgba(234,230,220,.55); border:1px solid rgba(234,230,220,.1); border-radius:4px; padding:1px 6px; white-space:nowrap';
const NOTE_R = M + '; font-size:11px; color:rgba(234,230,220,.6); line-height:1.6';
const LINK = 'color:#D9A648; text-decoration:none; ' + M + '; font-size:10.5px; letter-spacing:.06em';
...
export function riskSideChip(r) {
  ...
  const farbe = isSell ? '#DE8A7A' : isNo ? '#DE7E36' : '#D9A648';
...
export function riskCompositionBar(components, score) {
  ...
  return '<div style="margin-top:10px">'
    ...
    + '<div style="display:flex; height:10px; border-radius:4px; overflow:hidden; background:rgba(234,230,220,.07)">'
    + seg(groesse, 'rgba(234,230,220,.42)') + seg(timing, '#DE7E36') + seg(muster, '#D9A648')
    + '</div>'
    + tick(40, 'rgba(234,230,220,.28)') + tick(55, 'rgba(234,230,220,.28)') + tick(70, 'rgba(222,126,54,.6)')
    + '<div style="position:absolute; left:' + wert.toFixed(1) + '%; top:-3px; width:2px; height:16px; background:rgba(234,230,220,.9)"></div>'
    + '</div>'
    + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.55); margin-top:5px">size ' + groesse.toFixed(1) + ' · price &amp; timing ' + timing.toFixed(1) + ' · wallet pattern ' + muster.toFixed(1) + '</div>'
    + '</div>';
}
...
export function riskFlowRow(r) {
  ...
  const balken = gesamt > 0
    ? '<div style="display:flex; height:8px; border-radius:4px; overflow:hidden; background:rgba(234,230,220,.06); margin-bottom:5px">'
      + seg(kaufYes, '#D9A648') + seg(kaufNo, '#DE7E36') + seg(verkauf, '#DE8A7A') + '</div>'
    : '';
  const rest = gesamt > 0
    ? [['YES buys', kaufYes, '#69B47E'], ['NO buys', kaufNo, '#DE7E36'], ['sells', verkauf, '#DE8A7A']]
  ...
  return '<div style="margin-top:10px">' + balken
    ...
    + riskSideChip(r)
    + (rest ? '<span style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.55)">' + rest + '</span>' : '')
    + '</div></div>';
}
...
export function riskPriceStrip(r) {
  ...
  return '<div style="flex:1; min-width:0">'
    + '<div style="position:relative; height:8px; border-radius:4px; background:rgba(234,230,220,.06)">'
    + '<div style="position:absolute; left:' + links.toFixed(1) + '%; top:0; width:' + band.toFixed(1) + '%; height:8px; border-radius:4px; background:rgba(222,126,54,.4)"></div>'
    + '<div style="position:absolute; left:' + Math.max(0, Math.min(99, Number(r.price_last) * 100)).toFixed(1) + '%; top:-2px; width:2px; height:12px; background:#DE7E36"></div>'
    + '</div>'
    + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.55); margin-top:5px">at flag ' + esc(riskPriceLabel(r)) + bewegt + '</div>'
    + '</div>';
}
...
export function riskWindowStrip(r) {
  ...
  const ticks = offsets.length
    ? '<div style="position:relative; height:8px">'
      + offsets.map((o) => '<div style="position:absolute; left:' + Math.max(0, Math.min(98, (Number(o) || 0) * 98)).toFixed(1) + '%; top:0; width:2px; height:8px; background:rgba(234,230,220,.55)"></div>').join('')
      + '<div style="position:absolute; left:0; right:0; bottom:-2px; height:1px; background:rgba(234,230,220,.12)"></div>'
      + '</div>'
    : '';
  return '<div style="flex:1; min-width:0">' + ticks
    + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.55); margin-top:5px">' + esc(windowLabel(r.first_print, r.last_print, r.window_minutes)) + (r.prints ? ' · ' + r.prints + ' print' + (r.prints === 1 ? '' : 's') : '') + '</div>'
    + '</div>';
}
...
export function riskComponentsHtml(components) {
  ...
  return teile.map((c) => '<span style="' + CHIP + '">' + esc(c.label) + ' <span style="color:rgba(234,230,220,.85)">'
...
export function riskScoreBreakdown(components, score) {
  ...
  const bar = (c) => {
    ...
    const farbe = voll ? '#DE7E36' : 'rgba(234,230,220,.55)';
    const unter = [c.fact, c.rule].filter(Boolean).map((t) => esc(String(t))).join(' <span style="color:rgba(234,230,220,.5)">·</span> ')
      + (c.weight_note ? ' <span style="color:#DE7E36">' + esc(c.weight_note) + '</span>' : '');
    return '<div style="padding:6px 0; border-bottom:1px solid rgba(234,230,220,.05)">'
      + '<div style="display:flex; align-items:center; gap:10px">'
      + '<div style="flex:0 0 158px; font-size:12.5px; color:rgba(234,230,220,.85)" title="' + esc(String(c.measures || '')) + '">' + esc(String(c.label || c.key || '')) + '</div>'
      + '<div style="flex:1; height:6px; border-radius:4px; background:rgba(234,230,220,.08); overflow:hidden"><div style="width:' + (anteil * 100).toFixed(1) + '%; height:6px; background:' + farbe + '"></div></div>'
      + '<div style="flex:0 0 64px; text-align:right; ' + M + '; font-size:11.5px; color:' + (voll ? '#DE7E36' : 'rgba(234,230,220,.8)') + '">' + esc(String(c.value)) + '<span style="color:rgba(234,230,220,.55)">/' + esc(String(c.max)) + '</span></div>'
      + '</div>'
      + (unter ? '<div style="font-size:11px; color:rgba(234,230,220,.5); margin-top:3px; padding-left:0; line-height:1.45">' + unter + '</div>' : '')
      + '</div>';
  };
  const nichts = zero.length
    ? '<div style="font-size:11px; color:rgba(234,230,220,.6); padding:7px 0; line-height:1.5"><span style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.55)">NOT FOUND</span> '
      + zero.map((c) => esc(String(c.label || c.key || '')).toLowerCase() + (c.fact ? ' <span style="color:rgba(234,230,220,.5)">(' + esc(String(c.fact)) + ')</span>' : '')).join(' · ') + '</div>'
    : '';
  const kontext = ctx
    ? '<div style="display:flex; align-items:baseline; gap:10px; padding:7px 0; border-top:1px solid rgba(234,230,220,.05)">'
      + '<div style="flex:0 0 158px; font-size:12.5px; color:rgba(234,230,220,.85)">' + esc(String(ctx.label || 'Context')) + '</div>'
      + '<div style="flex:1; font-size:11px; color:rgba(234,230,220,.5); line-height:1.45">' + esc(String(ctx.fact || '')) + (ctx.rule ? ' <span style="color:rgba(234,230,220,.5)">· ' + esc(String(ctx.rule)) + '</span>' : '') + '</div>'
      + '<div style="flex:0 0 64px; text-align:right; ' + M + '; font-size:11.5px; color:' + (faktor > 1 ? '#DE7E36' : faktor < 1 ? 'rgba(234,230,220,.5)' : 'rgba(234,230,220,.8)') + '">×' + esc(String(ctx.value)) + '</div></div>'
    : '';
  ...
  const rechnung = '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.55); padding-top:8px; border-top:1px solid rgba(234,230,220,.08); margin-top:2px">'
    + summe.toFixed(1) + ' pts' + (ctx ? ' × ' + esc(String(ctx.value)) : '') + ' = <span style="color:#EAE6DC">' + produkt + '</span> / 100'
    + (stimmt ? '' : ' <span style="color:#DE7E36">· the card says ' + esc(String(score)) + ' — parts missing from this answer</span>')
    + ' <span style="color:rgba(234,230,220,.55)">· under 40 low · 40–54 elevated · 55–69 medium · 70+ high</span></div>';
...
export function riskWalletsHtml(wallets, count) {
  ...
  return wallets.map((w) => {
    ...
    return w.url
      ? '<a data-stop href="' + esc(w.url) + '" target="_blank" rel="noopener" style="' + CHIP + '; color:#D9A648; text-decoration:none">' + label + ' ↗</a>'
...
const BOOK_FARBE = (rel) => rel === 'adds' || rel === 'new_bet' ? '#D9A648' : rel === 'reduces' || rel === 'hedge' || rel === 'exit' ? '#DE7E36' : 'rgba(234,230,220,.6)';
...
export function riskBookSummary(T, r) {
  ...
  if (!eintrag) return '';
  const kopf = '<span style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">BOOK NOW</span> ';
  const grau = '<span style="font-size:11.5px; color:rgba(234,230,220,.6)">';
  ...
  if (ungelesen) teile.push('<span style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6)">' + ungelesen + ' not read</span>');
  return '<div style="margin-top:8px; display:flex; align-items:center; gap:8px; flex-wrap:wrap">' + kopf + teile.join('<span style="color:rgba(234,230,220,.5)">·</span>') + '</div>';
}
...
export function riskBookHtml(T, r) {
  const eintrag = riskBookEntry(T, r);
  if (!eintrag) return '';
  const kopf = '<span style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">WALLET BOOK NOW</span> ';
  if (eintrag.herkunft === 'loading') return '<div style="margin-top:8px; font-size:11.5px; color:rgba(234,230,220,.6)">' + kopf + 'reading the wallets\' open positions in this market…</div>';
  if (eintrag.herkunft === 'fehler') return '<div style="margin-top:8px; font-size:11.5px; color:rgba(234,230,220,.6)">' + kopf + 'not read (' + esc(eintrag.fehler || 'no answer') + ')</div>';
  const books = eintrag.data && Array.isArray(eintrag.data.wallets) ? eintrag.data.wallets : [];
  if (!books.length) return '<div style="margin-top:8px; font-size:11.5px; color:rgba(234,230,220,.6)">' + kopf + 'no wallet readable</div>';
  return '<div style="margin-top:8px; display:flex; flex-direction:column; gap:4px">'
    + books.map((b) => {
      if (!b.read) return '<div style="font-size:11.5px; color:rgba(234,230,220,.6)">' + kopf + esc(b.short || b.wallet) + ' not read (' + esc(b.error || 'no answer') + ')</div>';
      const netz = b.net === 'YES' || b.net === 'NO' ? 'net ' + b.net : b.net === 'balanced' ? 'balanced' : 'flat';
      return '<div style="font-size:11.5px; line-height:1.45; color:rgba(234,230,220,.7)">' + kopf
        + '<span style="' + M + '; color:rgba(234,230,220,.85)">' + esc(b.short || b.wallet) + '</span> '
...
function afterCell(after, key, label) {
  ...
  if (!after) return '<div><div style="' + HEAD_CELL + '">' + label + '</div><div style="' + M + '; font-size:12px; color:rgba(234,230,220,.6); margin-top:2px">n/a</div></div>';
  if (!p) return '<div><div style="' + HEAD_CELL + '">' + label + '</div><div style="' + M + '; font-size:12px; color:rgba(234,230,220,.6); margin-top:2px">not yet</div></div>';
  const move = p.move_c == null ? '' : ' <span style="color:' + (p.move_c > 0 ? '#D9A648' : p.move_c < 0 ? '#DE8A7A' : 'rgba(234,230,220,.5)') + '">' + (p.move_c > 0 ? '+' : '') + esc(String(p.move_c)) + '</span>';
...
export function renderRiskLog(T) {
  ...
  const intro = '<div style="padding:14px 24px 0; font-size:13px; color:rgba(234,230,220,.6); max-width:820px; line-height:1.5">'
  ...
  if (!live) {
    return intro + '<div style="display:flex; align-items:center; gap:10px; padding:16px 24px">'
      ...
      + '<span style="' + M + '; font-size:11px; letter-spacing:.08em; color:#DE7E36">loading /api/risk/log</span>'
      + '<span style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6)">reads the log and, for the newest Polymarket flags, the price afterwards</span></div>';
  }
  if (live._quelle === 'fehler') {
    return intro + '<div style="display:flex; align-items:center; gap:12px; padding:16px 24px">'
      + '<span style="' + M + '; font-size:11px; color:#DE8A7A">' + esc(herkunftSatz({ quelle: 'fehler', fehler: live._fehler }, '/api/risk/log')) + '</span>'
      + (T.neuLaden ? '<div ' + T.act(() => T.neuLaden('riskLog', 'risk')) + ' class="hv-bd32" style="' + M + '; font-size:11px; color:rgba(234,230,220,.7); border:1px solid rgba(234,230,220,.16); border-radius:4px; padding:5px 10px; cursor:pointer; white-space:nowrap">Try again</div>' : '')
      + '</div>';
  }
  ...
  const kopf = '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); padding:12px 24px 0">' + rows.length + ' flag' + (rows.length === 1 ? '' : 's')
  ...
  return intro + kopf + '<div style="padding:12px 24px 18px; display:grid; gap:12px">'
    + rows.map((f) => {
      const sevColor = f.sev === 'high' ? '#DE7E36' : f.sev === 'medium' ? 'rgba(234,230,220,.72)' : 'rgba(234,230,220,.5)';
      ...
      return '<div style="background:#131311; border:1px solid rgba(234,230,220,.09); border-radius:6px; padding:14px 18px">'
        + '<div style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px">'
        + '<div style="min-width:0">'
        + '<div style="' + M + '; font-size:11px; letter-spacing:.12em; color:rgba(234,230,220,.6)">' + esc(utcShort(f.last_seen)) + ' UTC · ' + esc(String(f.venue || '')) + (f.category ? ' · ' + esc(f.category) : '') + (f.times_seen > 1 ? ' · seen ' + f.times_seen + '× since ' + esc(utcShort(f.first_seen)) : '') + '</div>'
        + '<div style="font-size:14.5px; margin-top:5px; line-height:1.35">' + esc(f.title || f.market_key || '—') + (f.url ? ' ' + marketLink(f.url) : '') + '</div>'
        + '</div>'
        + '<div style="display:flex; align-items:baseline; gap:5px; flex:none"><div style="' + scoreStyle + '">' + (f.score != null ? Math.round(f.score) : '—') + '</div><div style="' + M + '; font-size:11px; color:rgba(234,230,220,.55)">/100</div></div>'
        + '</div>'
        ...
        + (hatAfter ? '<div style="display:flex; gap:22px; margin-top:10px">' + afterCell(f.after, '30m', '+30 MIN') + afterCell(f.after, '2h', '+2 H') + afterCell(f.after, '24h', '+24 H')
          + '<div><div style="' + HEAD_CELL + '">MEASURED ON</div><div style="' + M + '; font-size:12px; color:rgba(234,230,220,.55); margin-top:2px">' + (f.after ? esc(f.price_outcome || 'flagged') + ' side, from last print' : (String(f.venue).toLowerCase() === 'kalshi' ? 'Kalshi: no history read' : 'no history / not in the enriched set')) + '</div></div></div>' : '')
...
// re-render keeps it.
const BAND = (score) => score >= 70 ? ['HIGH', '#DE7E36'] : score >= 55 ? ['MEDIUM', '#DE7E36'] : score >= 40 ? ['ELEVATED', 'rgba(234,230,220,.7)'] : ['LOW', 'rgba(234,230,220,.6)'];
...
export function riskEventCard(T, r0) {
  ...
  const toggle = hatDetails
    ? '<div data-stop ' + T.act(() => T.setState({ riskOpen: Object.assign({}, s.riskOpen || {}, { [key]: !offen }) })) + ' class="hv-bd32" style="' + M + '; font-size:10.5px; letter-spacing:.06em; color:' + (offen ? '#EAE6DC' : 'rgba(234,230,220,.6)') + '; border:1px solid rgba(234,230,220,' + (offen ? '.3' : '.14') + '); border-radius:4px; padding:4px 9px; cursor:pointer; white-space:nowrap; user-select:none">' + (offen ? 'Why ' + score + '? ▴' : 'Why ' + score + '? ▾') + '</div>'
    : '';
  ...
  const details = offen && hatDetails
    ? '<div data-stop style="margin-top:12px; border-top:1px dashed rgba(234,230,220,.1); padding-top:10px; cursor:default">'
      + '<div style="display:flex; align-items:baseline; justify-content:space-between; gap:10px; flex-wrap:wrap">'
      + '<div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">WHY ' + score + ' / 100 · WHAT EACH PART SAW</div>'
      + (flags.length ? '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6)">flags: ' + flags.map((f) => esc(String(f))).join(' · ') + '</div>' : '')
      + '</div>'
  ...
  const streifen = (preisSpalte || fensterSpalte)
    ...
    : '';
  return '<div ' + (klickbar ? r.act + ' class="hv-bd20" ' : '') + 'data-bg style="background:#131311; border:1px solid ' + (r0.sev === 'high' ? 'rgba(222,126,54,.3)' : 'rgba(234,230,220,.09)') + '; border-radius:6px; padding:16px 18px; ' + (klickbar ? 'cursor:pointer; ' : '') + '">'
    + '<div style="display:flex; align-items:flex-start; justify-content:space-between; gap:12px">'
    ...
    + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.55)">/100</div></div>'
    ...
    + '<div style="' + M + '; font-size:11px; letter-spacing:.1em; color:rgba(234,230,220,.6); margin-top:4px"' + (r0.context_note ? ' title="' + esc(r0.context_note) + '"' : '') + '>' + (r0.category ? esc(String(r0.category).toUpperCase()) + ' · ' : '') + esc(String(r.venue || '').toUpperCase()) + '</div>'
    ...
    + (hatFlow
      ...
      : '<div style="font-size:13px; color:rgba(234,230,220,.6); margin-top:7px; line-height:1.45">' + esc(r.detail) + '</div>')
    + '<div style="height:1px; background:rgba(234,230,220,.07); margin:14px 0 12px"></div>'
    ...
    + [['WALLETS', r.wallets], ['NOTIONAL', r.notional], ['WINDOW', r.window]].map((p) =>
      '<div><div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">' + p[0] + '</div><div style="' + M + '; font-size:14px; margin-top:3px">' + esc(String(p[1])) + '</div></div>'
    ).join('')
    + '</div>' + toggle + '</div>'
    + details
    + '</div>';
...
export function renderRisk(T) {
  ...
  const trichterZeile = (label, wert, farbe, balkenFarbe, satz) =>
    ...
    + '<div style="flex:0 0 158px; ' + M + '; font-size:11px; letter-spacing:.08em; color:' + farbe + '">' + label + ' <span style="font-size:13px; color:' + (farbe === '#DE7E36' ? '#DE7E36' : '#EAE6DC') + '">' + (wert != null ? wert : '—') + '</span></div>'
    + '<div style="flex:1; height:12px; border-radius:4px; background:rgba(234,230,220,.07); overflow:hidden"><div style="width:' + trichterBreite(wert).toFixed(1) + '%; height:12px; background:' + balkenFarbe + '"></div></div>'
    + '<div style="flex:0 0 210px; font-size:11px; color:rgba(234,230,220,.5)">' + satz + '</div>'
    + '</div>';
  const seitenKpi = (label, wert, amber) =>
    '<div style="flex:1; background:#131311; border:1px solid rgba(234,230,220,.09); border-radius:6px; padding:12px 16px; display:flex; align-items:center; justify-content:space-between; gap:10px">'
    + '<div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">' + label + '</div>'
    + '<div style="' + M + '; font-size:18px; color:' + (amber ? '#DE7E36' : '#EAE6DC') + '">' + (wert != null ? wert : '—') + '</div></div>';
  const trichter =
    '<div style="display:flex; gap:14px; padding:14px 24px; border-bottom:1px solid rgba(234,230,220,.09)">'
    + '<div style="flex:1; background:#131311; border:1px solid rgba(234,230,220,.09); border-radius:6px; padding:14px 18px; display:flex; flex-direction:column; gap:9px">'
    + '<div style="display:flex; justify-content:space-between; align-items:baseline; gap:10px">'
    + '<div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">THE SCREEN, AS A FUNNEL</div>'
    + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.45)">floor at ' + minScore + '/100</div></div>'
    + trichterZeile('SCREENED', screened, 'rgba(234,230,220,.72)', 'rgba(234,230,220,.18)', 'every market with whale flow in the window')
    + trichterZeile('FLAGGED ≥ ' + minScore, flagged, 'rgba(234,230,220,.72)', 'rgba(234,230,220,.5)', 'cleared the flag threshold — these get cards')
    + trichterZeile('HIGH ≥ 70', hochEvents, '#DE7E36', '#DE7E36', 'strongest insider-like pattern')
    + '</div>'
  ...
  if (s.riskView === 'events') {
    ...
    const swatch = (farbe, wort) => '<div style="display:flex; align-items:center; gap:7px"><div style="width:14px; height:8px; border-radius:2px; background:' + farbe + '"></div><div style="font-size:11.5px; color:rgba(234,230,220,.72)">' + wort + '</div></div>';
    const legende = '<div style="display:flex; align-items:center; justify-content:space-between; gap:16px; padding:14px 24px 0; flex-wrap:wrap">'
      ...
      + '<span style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">SCORE COMPOSITION</span>'
      + swatch('rgba(234,230,220,.42)', 'size of the money')
      + swatch('#DE7E36', 'price &amp; timing')
      + swatch('#D9A648', 'wallet pattern')
      + '</div>'
      + '<div style="font-size:11.5px; color:rgba(234,230,220,.5)">ticks at 40 · 55 · 70 — low → elevated → medium → high · open <span style="' + M + '; font-size:11px">Why?</span> on a card for the full arithmetic</div>'
      + '</div>';
    ...
    const unterNote = riskFiltered.length && unterZahl > 0
      ? '<div style="padding:0 24px 18px; ' + M + '; font-size:11px; color:rgba(234,230,220,.55)">' + unterZahl + ' more market' + (unterZahl === 1 ? '' : 's') + ' screened below ' + minScore + '/100 — watch only, no card.</div>'
      : '';
    body = '<div>'
      + legende
    ...
    body = '<div>'
      + '<div style="padding:14px 24px 0; font-size:12.5px; color:rgba(234,230,220,.62); line-height:1.55; max-width:860px">'
      ...
      + '<div style="display:grid; grid-template-columns:' + GRID_W + '; gap:10px; padding:9px 16px; background:#131311; border-bottom:1px solid rgba(234,230,220,.09); ' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">'
      ...
      + walletRows.map((w) => {
        ...
        const scoreStyle = M + '; font-size:12px; border-radius:4px; padding:3px 9px; ' + (w.score >= 70 ? 'color:#0B0B0A; background:#DE7E36' : w.score >= 55 ? 'color:#DE7E36; border:1px solid rgba(222,126,54,.35)' : 'color:rgba(234,230,220,.7); border:1px solid rgba(234,230,220,.18)');
        ...
        const flagChips = flags.map((f) => {
          ...
          return '<span style="' + M + '; font-size:10.5px; white-space:nowrap; border-radius:4px; padding:1px 6px; '
            + (echt ? 'color:#DE7E36; border:1px solid rgba(222,126,54,.3)' : 'color:rgba(234,230,220,.6); border:1px solid rgba(234,230,220,.12)') + '">' + esc(f) + '</span>';
        }).join('');
        return '<div ' + T.act(() => T.openWallet(w.wallet, w.address)) + ' class="hv-panel" style="display:grid; grid-template-columns:' + GRID_W + '; gap:10px; align-items:center; padding:12px 16px; border-bottom:1px solid rgba(234,230,220,.06); ' + M + '; font-size:12.5px; cursor:pointer">'
          ...
          + '<div style="font-size:11px; color:rgba(234,230,220,.62); margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis" title="' + esc(w.context) + '">mostly in ' + esc(w.context) + '</div>'
          ...
          + '<div style="text-align:right; color:rgba(234,230,220,.6)">' + w.prints + '</div>'
          + '<div style="text-align:right">' + esc(String(w.notional)) + '</div>'
          + '<div style="text-align:right; color:rgba(234,230,220,.6)">' + esc(String(w.largest || '—')) + '</div>'
          + '<div style="text-align:right; color:rgba(234,230,220,.6)">' + esc(w.firstSeen) + '</div></div>';
      }).join('')
      + '</div></div>';
    ...
    const antwortDa = !!live && live._quelle !== 'fehler';
    const seiteFarbe = (seite) => seite === 'YES' ? '#D9A648' : seite === 'NO' ? '#DE8A7A' : 'rgba(234,230,220,.7)';
    body = '<div>'
      + '<div style="padding:14px 24px 0; font-size:12.5px; color:rgba(234,230,220,.62); line-height:1.55; max-width:860px">'
      ...
      + freshRows.map((c) => {
        ...
        return '<div style="background:#131311; border:1px solid rgba(234,230,220,.09); border-radius:6px; padding:16px 18px">'
          + '<div style="font-size:14.5px; line-height:1.35">' + esc(c.market) + (c.venue ? ' <span style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6)">' + esc(String(c.venue).toUpperCase()) + '</span>' : '') + '</div>'
          + '<div style="display:flex; gap:22px; margin-top:12px; align-items:flex-end; flex-wrap:wrap">'
          + '<div><div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">FRESH WALLETS</div>'
          + '<div style="display:flex; align-items:baseline; gap:8px; margin-top:3px"><span style="' + M + '; font-size:20px; color:#DE7E36">' + (c.count != null ? c.count : '—') + '</span>'
          + '<span style="' + M + '; font-size:11px; color:#DE7E36; letter-spacing:2px">' + punkte + '</span></div></div>'
          + '<div><div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">ALL ON</div>'
          + '<div style="' + M + '; font-size:16px; margin-top:3px; color:' + seiteFarbe(c.side) + '">' + esc(c.side || 'same side') + '</div></div>'
          + '<div><div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">COMBINED</div>'
          + '<div style="' + M + '; font-size:16px; margin-top:3px">' + esc(String(c.notional || '—')) + '</div></div>'
          + '</div>'
          + '<div style="font-size:11.5px; color:rgba(234,230,220,.62); margin-top:10px; line-height:1.45">' + esc(c.detail) + '</div>'
          + '</div>';
      }).join('')
      + '</div></div>';
    ...
    body = '<div>'
      ...
      + '</div>'
      + '<div style="border:1px solid rgba(234,230,220,.09); border-radius:6px; margin:14px 24px; overflow:hidden">'
      + '<div style="display:grid; grid-template-columns:' + GRID_T + '; gap:10px; padding:9px 16px; background:#131311; border-bottom:1px solid rgba(234,230,220,.09); ' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">'
      ...
      + timingRows.map((c) => {
        ...
        const balken = '<div style="display:flex; align-items:center; gap:8px">'
          + '<div style="flex:1; height:7px; border-radius:4px; background:rgba(234,230,220,.08); overflow:hidden">'
          + '<div style="width:' + (anteil * 100).toFixed(1) + '%; height:7px; background:' + (c.same ? '#DE7E36' : 'rgba(234,230,220,.45)') + '"></div></div>'
          + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.7); white-space:nowrap">' + esc(c.window) + '</div></div>';
        return '<div style="display:grid; grid-template-columns:' + GRID_T + '; gap:10px; align-items:center; padding:12px 16px; border-bottom:1px solid rgba(234,230,220,.06); ' + M + '; font-size:12.5px">'
          ...
          + balken
          + '<div style="text-align:right; ' + M + '; font-size:11.5px; color:' + (c.same ? '#DE7E36' : 'rgba(234,230,220,.6)') + '">' + (c.same ? 'all on ' + esc(c.side || 'one side') : 'mixed') + '</div>'
    ...
    const musterFarbe = (p) => p === 'Tight clique' ? '#DE7E36' : p === 'Connected group' ? 'rgba(234,230,220,.8)' : 'rgba(234,230,220,.55)';
    body = '<div style="padding:16px 24px">'
      ...
      + networkRows.map((n) => {
        ...
        return '<div style="background:#131311; border:1px solid rgba(234,230,220,.09); border-left:3px solid ' + farbe + '; border-radius:6px; padding:16px 18px">'
          ...
          + '<span style="' + M + '; font-size:11px; color:rgba(234,230,220,.6)">' + n.size + ' wallets</span></div>'
          + (n.pattern ? '<span style="' + M + '; font-size:10.5px; letter-spacing:.1em; color:' + musterFarbe(n.pattern) + '; border:1px solid rgba(234,230,220,.14); border-radius:4px; padding:2px 7px">' + esc(String(n.pattern).toUpperCase()) + '</span>' : '')
          + '</div>'
          + '<div style="font-size:12.5px; color:rgba(234,230,220,.62); margin-top:8px; line-height:1.45">' + esc(n.story) + '</div>'
          + (mitglieder.length
            ? '<div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6); margin-top:11px">WHO</div>'
              + '<div style="display:flex; gap:5px; flex-wrap:wrap; margin-top:5px">'
              + mitglieder.map((mm) => '<span ' + T.act(() => T.openWallet(mm.kurz, mm.wallet)) + ' class="hv-bd32" style="' + M + '; font-size:11px; color:rgba(234,230,220,.8); border:1px solid rgba(234,230,220,.16); border-radius:4px; padding:3px 8px; cursor:pointer" title="open this wallet">' + esc(mm.kurz) + '</span>').join('')
              + (rest > 0 ? '<span style="' + M + '; font-size:11px; color:rgba(234,230,220,.55); padding:3px 2px">+ ' + rest + ' more</span>' : '')
              + '</div>'
            : '')
          + (maerkte.length
            ? '<div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6); margin-top:11px">WHERE THEY MET</div>'
              ...
              + maerkte.map((mk) => '<div style="display:flex; gap:10px; align-items:baseline; font-size:12px">'
                + '<span style="color:rgba(234,230,220,.75); min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap; flex:1" title="' + esc(mk.title) + '">' + esc(mk.title) + '</span>'
                + '<span style="' + M + '; font-size:11.5px; white-space:nowrap">' + esc(mk.label || '') + '</span></div>').join('')
              + '</div>'
            : '')
          + '<div style="display:flex; gap:20px; margin-top:12px">'
          + '<div><div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">SHARED MARKETS</div><div style="' + M + '; font-size:14px; margin-top:3px">' + esc(n.shared) + '</div></div>'
          + '<div><div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">PAIRED NOTIONAL</div><div style="' + M + '; font-size:14px; margin-top:3px">' + esc(n.notional) + '</div></div>'
          + '</div></div>';
      }).join('')
      + '</div></div>';
  }
  ...
  return '<div>'
    ...
    + '<div style="display:flex; align-items:flex-end; justify-content:space-between; gap:20px">'
    + '<div><div style="' + M + '; font-size:11px; letter-spacing:.18em; color:#DE7E36">RISK SCREEN</div>'
    ...
    + (live && live.as_of ? '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.55)">as of ' + esc(String(live.as_of)) + '</div>' : '')
    + '</div>'
    + '<div style="font-size:13px; color:rgba(234,230,220,.55); margin-top:10px; max-width:760px">Best-effort screen on public trade data — research leads, not legal findings. Sports odds, crypto &amp; market prices, and weather are excluded: game results, exchange prices and weather models cannot be traded on early.</div>'
    + '<div style="display:flex; gap:7px; margin-top:12px; flex-wrap:wrap">'
    + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); border:1px solid rgba(234,230,220,.12); border-radius:4px; padding:4px 9px">UNDER 40 · LOW</div>'
    + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.6); border:1px solid rgba(234,230,220,.18); border-radius:4px; padding:4px 9px">40–54 · ELEVATED</div>'
    + '<div style="' + M + '; font-size:11px; color:#DE7E36; border:1px solid rgba(222,126,54,.3); border-radius:4px; padding:4px 9px">55–69 · MEDIUM</div>'
    + '<div style="' + M + '; font-size:11px; color:#0B0B0A; background:#DE7E36; border-radius:4px; padding:4px 9px">70 AND UP · HIGH</div>'
    + '</div></div>'

    + trichter

    + (laedt
      ? '<div style="display:flex; align-items:center; gap:10px; padding:12px 24px; border-bottom:1px solid rgba(234,230,220,.09); background:#131311">'
        + '<span style="width:7px; height:7px; border-radius:50%; background:#DE7E36; display:inline-block"></span>'
        + '<span style="' + M + '; font-size:11px; letter-spacing:.08em; color:#DE7E36">building the day\'s tape, ~90 s on a cold cache</span>'
        + '<span style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6)">/api/risk pages a day of prints and looks up market categories; the second visit is instant</span></div>'
      : (T.herkunft.risks && T.herkunft.risks.quelle === 'fehler' && T.neuLaden
        ...
        ? '<div style="display:flex; align-items:center; gap:12px; padding:12px 24px; border-bottom:1px solid rgba(234,230,220,.09); background:#131311">'
          + '<span style="' + M + '; font-size:11px; color:#DE8A7A">' + esc(risikoSatz) + '</span>'
          + '<div ' + T.act(() => T.neuLaden('risk', 'risk')) + ' class="hv-bd32" style="' + M + '; font-size:11px; color:rgba(234,230,220,.7); border:1px solid rgba(234,230,220,.16); border-radius:4px; padding:5px 10px; cursor:pointer; white-space:nowrap">Try again</div></div>'
        : ''))
  ...
  if (live && live.watchlist && live.watchlist.length) {
    return live.watchlist.map((item) => {
      ...
      return {
        ...
        sparkPoints: '', color: 'rgba(234,230,220,.3)', priceLabel: '—', changeLabel: '—',
        changeStyle: M + '; font-size:13px; text-align:right; color:rgba(234,230,220,.6)',
        volLabel: '—', ends: '—', act: ''
      };
    });
  }
...
export function renderTrack(T) {
  ...
  return '<div>'
    + '<div style="padding:20px 24px 16px; border-bottom:1px solid rgba(234,230,220,.09)">'
    + '<div style="' + M + '; font-size:11px; letter-spacing:.18em; color:#D9A648">TRACKED</div>'
    ...
    + '<div style="padding:18px 24px; border-bottom:1px solid rgba(234,230,220,.09)">'
    + '<div style="' + M + '; font-size:10.5px; letter-spacing:.14em; color:rgba(234,230,220,.55); margin-bottom:13px">WALLETS YOU FOLLOW</div>'
    + (cards.length ? '<div style="display:grid; grid-template-columns:repeat(3,1fr); gap:14px">'
    + cards.map((w) => {
      const gradeStyle = M + '; font-size:12px; border-radius:4px; padding:2px 9px; ' + (w.grade === 'A' || w.grade === 'A+' ? 'color:#0B0B0A; background:#D9A648' : w.grade === '—' ? 'color:rgba(234,230,220,.6); border:1px solid rgba(234,230,220,.14)' : 'color:#DE7E36; border:1px solid rgba(222,126,54,.35)');
      const pnlText = typeof w.pnlRaw === 'number' && T.liveData.track ? (w.pnlRaw >= 0 ? '+' : '-') + money(Math.abs(w.pnlRaw)) : w.pnl;
      const pnlStyle = M + '; font-size:14px; margin-top:3px; color:' + (String(pnlText).charAt(0) === '+' ? '#69B47E' : String(pnlText).charAt(0) === '-' ? '#D95F52' : 'rgba(234,230,220,.5)');
      return '<div ' + T.act(() => T.openWallet(w.openAs)) + ' class="hv-bd20" style="background:#131311; border:1px solid rgba(234,230,220,.09); border-radius:6px; padding:15px 17px; cursor:pointer">'
        + '<div style="display:flex; align-items:center; justify-content:space-between">'
        + '<div style="font-size:15px">' + esc(w.name) + '</div>'
        + '<div style="' + gradeStyle + '">' + esc(w.grade) + '</div></div>'
        + '<div style="' + M + '; font-size:10.5px; color:rgba(234,230,220,.6); margin-top:4px">' + esc(w.wallet) + '</div>'
        + '<div style="display:flex; gap:20px; margin-top:14px">'
        + '<div><div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">' + (w.pnlLabel || '30D PROFIT') + '</div><div style="' + pnlStyle + '">' + esc(String(pnlText)) + '</div></div>'
        + '<div><div style="' + M + '; font-size:10.5px; letter-spacing:.12em; color:rgba(234,230,220,.6)">LAST TRADE</div><div style="' + M + '; font-size:14px; margin-top:3px">' + esc(w.last) + '</div></div>'
        + '</div></div>';
    }).join('')
    + '</div>' : '<div style="' + M + '; font-size:11.5px; color:rgba(234,230,220,.6); padding:14px 0">No followed wallets reported by /api/track — the list lives in data/followed_wallets.json on the machine that runs the API, and this page only reads it.</div>')
    + '</div>'
    + '<div style="padding:18px 24px">'
    + '<div style="' + M + '; font-size:10.5px; letter-spacing:.14em; color:rgba(234,230,220,.55); margin-bottom:13px">MARKETS ON YOUR WATCHLIST</div>'
    + (watch.length ? '' : '<div style="' + M + '; font-size:11.5px; color:rgba(234,230,220,.6); padding:6px 0">'
