...
const BALKEN_FARBE = { gewinn: '#69B47E', kosten: '#D95F52', summe: '#6E9BC8' };
...
function achse(sk, dia, hoehe) {
  ...
  if (sk.min < 0 && sk.max > 0) {
    out += '<line x1="' + nullX + '" y1="6" x2="' + nullX + '" y2="' + (hoehe - 22)
      + '" stroke="rgba(234,230,220,.28)" stroke-width="1" />';
  }
  if (typeof dia.referenz === 'number' && dia.referenz !== 0) {
    const rx = sk.x(dia.referenz);
    out += '<line x1="' + rx + '" y1="6" x2="' + rx + '" y2="' + (hoehe - 22)
      + '" stroke="rgba(234,230,220,.35)" stroke-width="1" stroke-dasharray="4 4" />';
    if (dia.referenz_label) {
      out += '<text x="' + rx + '" y="' + (hoehe - 8) + '" fill="rgba(234,230,220,.6)" '
...
function labelText(text, y) {
  return '<text x="' + LABEL_X + '" y="' + (y + 4) + '" fill="rgba(234,230,220,.72)" font-size="11.5" '
...
export function diagramm(dia) {
  ...
  dia.punkte.forEach((p) => {
    if (gruppen && Array.isArray(p.werte)) {
      p.werte.forEach((w, i) => {
        const farbe = BALKEN_FARBE[p.art] || '#6E9BC8';
        const x0 = sk.x(0);
        const x1 = sk.x(w);
        ...
        koerper += wertText(fmtZahl(w), PLOT_R + 8, y, 'rgba(234,230,220,.75)');
        y += ZEILE;
      });
      return;
    }
    ...
    if (typeof p.von === 'number' && typeof p.bis === 'number') {
      ...
      const farbe = p.farbe || (beruehrt ? '#DE7E36' : '#6E9BC8');
      ...
      koerper += wertText(p.text || (fmtZahl(p.von) + ' … ' + fmtZahl(p.bis)), PLOT_R + 8, y, 'rgba(234,230,220,.75)');
    } else if (typeof p.wert === 'number') {
      const farbe = BALKEN_FARBE[p.art] || (p.wert < 0 ? '#D95F52' : '#69B47E');
      ...
      koerper += wertText(fmtZahl(p.wert), PLOT_R + 8, y, 'rgba(234,230,220,.78)');
    }
    y += ZEILE;
  });

  return '<div style="' + CARD + '; padding:14px 16px 10px">'
    + '<div style="' + M + '; font-size:11px; letter-spacing:.13em; color:rgba(234,230,220,.5); margin-bottom:4px">'
...
export const SERIEN_FARBEN = ['#D9A648', '#6E9BC8', '#69B47E', '#AC93C6', '#86BDB2', '#DE8A7A', '#D4B878', '#93A5C6', '#98978E'];
...
export function linien(k) {
  ...
  [0, 0.5, 1].forEach((f) => {
    ...
    raster += '<line x1="' + L + '" y1="' + y(w).toFixed(1) + '" x2="' + R + '" y2="' + y(w).toFixed(1)
      + '" stroke="rgba(234,230,220,' + (f === 0 ? '.18' : '.07') + ')" stroke-width="1" />'
      + '<text x="' + (L - 6) + '" y="' + (y(w) + 4).toFixed(1) + '" fill="rgba(234,230,220,.6)" font-size="11" '
      + 'font-family="IBM Plex Mono, monospace" text-anchor="end">' + esc(fmtZahl(Math.round(w * 1000) / 1000)) + '</text>';
  });
  let xLabels = '';
  k.x.forEach((label, i) => {
    xLabels += '<text x="' + x(i).toFixed(1) + '" y="' + (H - 10) + '" fill="rgba(234,230,220,.6)" font-size="11" '
  ...
  serien.forEach((s, si) => {
    ...
    legende += '<div style="display:flex; align-items:center; gap:6px; ' + M + '; font-size:11px; color:rgba(234,230,220,.65)">'
  ...
  return '<div style="' + CARD + '; padding:14px 16px 10px">'
    ...
    + '<div style="' + M + '; font-size:11px; letter-spacing:.13em; color:rgba(234,230,220,.5)">'
    + esc(k.titel || '') + (k.einheit ? ' · ' + esc(k.einheit) : '') + '</div>'
    + (k.hinweis ? '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.55)">' + esc(k.hinweis) + '</div>' : '')
    + '</div>'
...
export function kalibrierung(k) {
  ...
  punkte.slice().sort((a, b) => a.vorhergesagt - b.vorhergesagt).forEach((p, i) => {
    ...
    const farbe = daneben ? '#DE7E36' : '#6E9BC8';
  ...
  const achse = 'fill="rgba(234,230,220,.6)" font-size="10.5" font-family="IBM Plex Mono, monospace"';
  ...
  return '<div style="' + CARD + '; padding:12px 14px 8px">'
    + '<div style="' + M + '; font-size:11px; letter-spacing:.12em; color:rgba(234,230,220,.6)">' + esc(k.titel || '') + '</div>'
    + '<div style="' + M + '; font-size:11px; color:rgba(234,230,220,.55); margin-top:2px">' + esc(k.hinweis || ('n ' + gesamt + ' · ' + punkte.length + ' bins')) + '</div>'
    ...
    + '<rect x="' + PAD + '" y="' + PAD + '" width="' + (S - 2 * PAD) + '" height="' + (S - 2 * PAD) + '" fill="none" stroke="rgba(234,230,220,.1)" />'
    + '<line x1="' + PAD + '" y1="' + (S - PAD) + '" x2="' + (S - PAD) + '" y2="' + PAD + '" stroke="rgba(234,230,220,.3)" stroke-dasharray="3 3" />'
    + '<path d="' + linie + '" fill="none" stroke="rgba(234,230,220,.25)" stroke-width="1" />'
    + marken
...
export function pnlZeitkurve(k) {
  ...
  const farbe = k.farbe || (letzte >= 0 ? '#69B47E' : '#D95F52');
  ...
  ticks.forEach((tv) => {
    ...
    gitter += '<line x1="' + L + '" y1="' + yy.toFixed(1) + '" x2="' + R + '" y2="' + yy.toFixed(1) + '" stroke="rgba(234,230,220,.07)" stroke-width="1" />'
      + '<text x="' + (R + 8) + '" y="' + (yy + 3.5).toFixed(1) + '" fill="rgba(234,230,220,.6)" font-size="10.5" font-family="IBM Plex Mono, monospace">' + esc(kurzGeld(tv)) + '</text>';
  });
  let nulllinie = '';
  if (min < 0 && max > 0) {
    nulllinie = '<line x1="' + L + '" y1="' + y(0).toFixed(1) + '" x2="' + R + '" y2="' + y(0).toFixed(1)
      + '" stroke="rgba(234,230,220,.28)" stroke-width="1" stroke-dasharray="4 4" />';
  }
  ...
  for (let i = 0; i < nDatum; i += 1) {
    ...
    xLabels += '<text x="' + x(ms).toFixed(1) + '" y="' + (H - 9) + '" fill="rgba(234,230,220,.6)" font-size="10.5" '
  ...
  const endPunkt = '<circle cx="' + x(t1).toFixed(1) + '" cy="' + y(letzte).toFixed(1) + '" r="4" fill="' + farbe + '" stroke="#131311" stroke-width="2" />';
...
export function stepKurve(k) {
  ...
  if (min < 0 && max > 0) {
    nulllinie = '<line x1="' + L + '" y1="' + y(0).toFixed(1) + '" x2="' + R + '" y2="' + y(0).toFixed(1)
      + '" stroke="rgba(234,230,220,.25)" stroke-width="1" stroke-dasharray="4 4" />';
  }
  ...
  const xLabels = '<text x="' + L + '" y="' + (H - 8) + '" fill="rgba(234,230,220,.6)" font-size="11" '
    + 'font-family="IBM Plex Mono, monospace">' + esc(String(k.punkte[0].label || '')) + '</text>'
    + '<text x="' + R + '" y="' + (H - 8) + '" fill="rgba(234,230,220,.6)" font-size="11" '
