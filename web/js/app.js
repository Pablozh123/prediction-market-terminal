...
class Terminal {
  ...
  opt(label, active, patch) {
    const style = "font-family:'IBM Plex Mono',monospace; font-size:var(--t-micro); border-radius:var(--r-control); padding:5px 9px; cursor:pointer; " + (active ? 'color:var(--on-accent); background:var(--accent); font-weight:600' : 'color:rgba(var(--ink),.55); border:1px solid rgba(var(--ink),.14)');
  ...
  chip(label, active, patch) {
    const style = "font-family:'IBM Plex Mono',monospace; font-size:var(--t-micro); letter-spacing:.06em; border-radius:var(--r-control); padding:5px 10px; cursor:pointer; " + (active ? 'color:var(--on-accent); background:var(--accent); font-weight:600' : 'color:rgba(var(--ink),.55); border:1px solid rgba(var(--ink),.16)');
    return '<div ' + this.act(typeof patch === 'function' ? patch : () => this.setState(patch))
      + ' aria-pressed="' + (active ? 'true' : 'false') + '" style="' + style + '">' + esc(label) + '</div>';
  }

  tab(label, active, patch) {
    const style = 'font-size:var(--t-small); border-radius:var(--r-control); padding:7px 13px; cursor:pointer; ' + (active ? 'color:var(--on-accent); background:var(--accent); font-weight:600' : 'color:rgba(var(--ink),.6); border:1px solid rgba(var(--ink),.16)');
    return '<div ' + this.act(typeof patch === 'function' ? patch : () => this.setState(patch))
      + ' aria-pressed="' + (active ? 'true' : 'false') + '" style="' + style + '">' + esc(label) + '</div>';
  }
  ...
  stepper(valueLabel, onDown, onUp, size) {
    const s = size === 'lg';
    const btn = 'width:' + (s ? 32 : 28) + 'px; height:' + (s ? 34 : 32) + 'px; flex:none; border:1px solid rgba(var(--ink),.16); border-radius:' + (s ? 8 : 7) + 'px; display:flex; align-items:center; justify-content:center; font-family:\'IBM Plex Mono\',monospace; font-size:' + (s ? 15 : 14) + 'px; color:rgba(var(--ink),.7); cursor:pointer';
    const val = 'flex:1; background:var(--panel); border:1px solid rgba(var(--ink),.16); border-radius:' + (s ? 8 : 7) + 'px; padding:' + (s ? '8px 12px' : '7px 8px') + '; font-family:\'IBM Plex Mono\',monospace; font-size:' + (s ? 13 : 12.5) + 'px; text-align:center';
  ...
  changeStyle(chg) {
    return "font-family:'IBM Plex Mono',monospace; font-size:var(--t-body); text-align:right; color:" + (chg >= 0 ? 'var(--pos)' : 'var(--neg)');
  }
  ...
  tapeRowView(t) {
    ...
    return {
      ...
      sideStyle: "font-family:'IBM Plex Mono',monospace; font-size:var(--t-small); color:" + (t.side.indexOf('BUY') === 0 ? 'var(--pos)' : 'var(--neg)'),
  ...
  riskCardView(r) {
    ...
    return {
      ...
      kindStyle: "font-family:'IBM Plex Mono',monospace; font-size:var(--t-micro); letter-spacing:.12em; color:" + (r.sev === 'high' ? 'var(--warn)' : r.sev === 'medium' ? 'rgba(var(--ink),.66)' : 'rgba(var(--ink),.6)'),
      scoreStyle: "font-family:'IBM Plex Mono',monospace; font-size:var(--t-head); color:" + (r.sev === 'high' ? 'var(--warn)' : 'rgba(var(--ink),.72)'),
  ...
  navItem(id, label, badge, badgeColor) {
    ...
    const labelStyle = 'font-size:var(--t-body); color:' + (active ? 'var(--text)' : 'rgba(var(--ink),.62)') + '; font-weight:' + (active ? '600' : '400');
    const badgeStyle = badge ? ("font-family:'IBM Plex Mono',monospace; font-size:var(--t-micro); padding:1px 6px; border-radius:var(--r-control); " + (badgeColor === 'amber' ? 'color:var(--warn); border:1px solid rgba(var(--warn-rgb),.4)' : 'color:var(--on-accent); background:var(--accent)')) : 'display:none';
  ...
  renderSidebar() {
    ...
    const groupHtml = groups.map((g) =>
      ...
      + '<div style="font-family:\'IBM Plex Mono\',monospace; font-size:var(--t-micro); letter-spacing:.18em; padding:0 6px 6px; color:rgba(var(--ink),.55)">' + g.label + '</div>'
      + g.items.join('') + '</div>'
    ).join('');
    ...
    const foot = "font-family:'IBM Plex Mono',monospace; font-size:var(--t-micro); line-height:1.7; color:rgba(var(--ink),.6)";
    ...
    return ''
      ...
      + '<div style="font-family:\'IBM Plex Mono\',monospace; font-size:var(--t-small); color:rgba(var(--ink),.6); flex:1">Search</div>'
      + '<div style="font-family:\'IBM Plex Mono\',monospace; font-size:var(--t-micro); color:rgba(var(--ink),.55); border:1px solid rgba(var(--ink),.16); border-radius:var(--r-control); padding:0 5px">/</div></div>'
      + groupHtml
      ...
      + '<a href="https://polygonscan.com/address/' + esc(LIVE_RUN_WALLET_FULL) + '" target="_blank" rel="noopener" class="hv-accent" title="' + esc(LIVE_RUN_WALLET_FULL) + ' — view on-chain on Polygonscan" style="color:rgba(var(--ink),.7); text-decoration:underline dotted">' + esc(LIVE_RUN_WALLET) + ' ↗</a>'
      + (runsIdx >= 0
        ? ' · <span ' + this.act(() => this.goStudy(runsIdx)) + ' class="hv-accent" title="every bet on the Live runs page" style="color:rgba(var(--ink),.55); cursor:pointer; text-decoration:underline dotted">runs</span>'
        : '')
      + '</div></div>';
  }
  ...
  renderTopbar() {
    ...
    return ''
      ...
      + '<span style="font-family:\'IBM Plex Mono\',monospace; font-size:var(--t-micro); letter-spacing:.16em; color:rgba(var(--ink),.66)">' + liveLabel + ' · ' + s.clock + ' UTC</span></div>'
      + '<div style="display:flex; align-items:center; gap:14px">'
      + '<div style="font-family:\'IBM Plex Mono\',monospace; font-size:var(--t-micro); letter-spacing:.14em; color:rgba(var(--ink),.55)">MICROSTRUCTURE, MEASURED · READ-ONLY · NO ORDERS</div>'
      ...
      + '<div ' + this.act(() => this.toggleTheme()) + ' aria-label="Switch to ' + (s.theme === 'light' ? 'dark' : 'light') + ' theme" class="hv-bd30" style="font-family:\'IBM Plex Mono\',monospace; font-size:var(--t-micro); letter-spacing:.08em; border:1px solid rgba(var(--ink),.22); border-radius:var(--r-control); padding:4px 9px; cursor:pointer; color:rgba(var(--ink),.7); user-select:none">'
  ...
  mount() {
    ...
    document.addEventListener('mouseover', (e) => {
      ...
      tipEl.innerHTML = '<div style="display:flex; gap:10px; align-items:flex-start">'
        ...
        + tip.rows.map((r) => '<div style="display:flex; justify-content:space-between; gap:14px; font-size:var(--t-small)"><span style="color:rgba(var(--ink),.55)">' + esc(String(r[0])) + '</span><span style="' + M + '; color:' + (/^(unrealised|realised)$/.test(String(r[0])) ? farbe : 'var(--text)') + '; text-align:right">' + esc(String(r[1])) + '</span></div>').join('')
        + '</div>';
