...
export function renderDetail(T) {
  ...
  return '<div role="dialog" aria-modal="true" aria-label="' + esc(v.title || v.kicker || 'Detail') + '" style="position:absolute; top:0; right:0; bottom:0; width:392px; box-sizing:border-box; border-left:1px solid var(--line-2); background:var(--bg); overflow-y:auto; z-index:20; box-shadow:-24px 0 48px var(--shadow-45); animation:panelIn .22s ease-out">'
    ...
    + v.list.map((it) =>
      ...
      + '<div style="' + M + '; font-size:var(--t-micro); color:var(--ink-3); margin-top:var(--sp-2)">' + esc(it.secondary) + '</div></div>'
    ...
    + (v.primaryAction && v.primaryAct
      ? '<div style="display:flex; flex-direction:column; gap:var(--sp-3); margin-top:var(--sp-6)">'
        + '<div ' + v.primaryAct + ' class="hv-accentbg" style="font-size:var(--t-body); font-weight:600; text-align:center; color:var(--on-accent); background:var(--accent); border-radius:var(--r-control); padding:var(--sp-4); cursor:pointer">' + esc(v.primaryAction) + '</div>'
        + (v.secondaryAction && v.secondaryAct
          ? '<div ' + v.secondaryAct + ' class="hv-edge-strong" style="font-size:var(--t-small); text-align:center; color:var(--ink-2); border:1px solid var(--line-1); border-radius:var(--r-control); padding:var(--sp-4); cursor:pointer">' + esc(v.secondaryAction) + '</div>'
          : '')
        + '</div>'
      : '')
    + '</div></div>';
...
export function renderSearch(T) {
  ...
  return '<div ' + T.act(() => T.setState({ searchOpen: false }), { plain: true }) + ' data-bg style="position:fixed; inset:0; background:var(--scrim); display:flex; align-items:flex-start; justify-content:center; padding-top:14vh; z-index:50">'
    ...
    + results.map((r, i) =>
      '<div ' + r.act + ' data-result="' + i + '" class="hv-el" style="display:flex; align-items:center; gap:var(--sp-4); padding:var(--sp-4) var(--sp-6); border-bottom:1px solid var(--line-3); cursor:pointer' + (i === 0 ? '; background:var(--panel-hover)' : '') + '">'
      ...
      + '<div style="' + M + '; font-size:var(--t-micro); color:var(--ink-3); margin-top:var(--sp-1)">' + esc(r.meta) + '</div></div>'
      + '<div style="' + M + '; font-size:var(--t-small); color:var(--ink-3)">' + r.value + '</div></div>'
    ).join('')
    + '</div>'
    + '<div style="padding:var(--sp-4) var(--sp-6); ' + M + '; font-size:var(--t-micro); color:var(--ink-4); display:flex; gap:var(--sp-5)">'
