...
- **Systemuhr dieses Rechners synchronisiert NICHT** (W32Time "free-running", live +68s vor; `w32tm /resync` braucht Admin). Folge: alle rohen Latenz-Vergleiche local-vs-exchange sind um den Offset verschoben. Der Daemon misst deshalb `ct.measure_clock_offset_seconds()` (CLOB `/time`) beim Start + alle 30 min, korrigiert die ws_worker-Latenz und schreibt `clock_offset_seconds` ins Status-JSON. **Vor Live-Trading zwingend NTP fixen** (CLOB-Order-Signaturen sind zeitstempel-sensitiv): Admin-PowerShell → `Start-Service w32time; w32tm /resync /force`.
