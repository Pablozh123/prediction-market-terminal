...
- **Systemuhr:** Auf der Dev-Maschine stand W32Time auf `Type=NoSync` (synchronisierte NIE; Uhr war +68s vor und verfälschte alle Latenz-Telemetrie). **Gefixt 2026-06-12:** Admin-Shell → `w32tm /config /syncfromflags:manual /manualpeerlist:"time.cloudflare.com,0x9 time.windows.com,0x9 pool.ntp.org,0x9" /update; Restart-Service w32time; w32tm /resync /force` (ein blankes `/resync` scheitert bei NoSync mit "keine Zeitdaten verfügbar"). Auf jeder neuen Maschine prüfen: `w32tm /query /status` + Registry `W32Time\Parameters\Type` muss `NTP` sein — CLOB-Order-Signaturen sind zeitstempel-sensitiv. Unabhängig davon misst der Daemon `ct.measure_clock_offset_seconds()` (CLOB `/time`) beim Start + alle 30 min, korrigiert die ws_worker-Latenz und schreibt `clock_offset_seconds` ins Status-JSON.
