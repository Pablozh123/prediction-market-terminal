...
**Signal-conditioned quoting.** Total PnL improved 18%. Markout per fill did not
move at all. The gain came only from placing fewer quotes, and losing less by
trading less is not an edge. The per-fill metric exists to catch exactly that.

**Signed order flow.** 51.3% hit rate, no usable edge. Published work later
