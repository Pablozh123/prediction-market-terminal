...
assert.match(renderFed(T), /Noch keine/);
const actions = [];
let reloads = 0;
...
assert.match(html, /30 Tage/);
assert.match(html, /7 Tage/);
assert.match(html, /1 Tag/);
assert.match(html, /&lt;script&gt;/);
assert.doesNotMatch(html, /<script>/);
assert.match(html, /CME FedWatch/);
assert.match(chart([{name:'a', points: [{t: 900000,p:.5}], color:'var(--s1)', maxAge:7200}], 1000000, 1, false), /Keine Beobachtungen/);
