...
- **Mentions bot, live.** Podcast and earnings-call "will X be said" markets: a content-drop prober, GPU transcription in 20-second chunks, speaker attribution, and a decision layer that buys YES only after the live count has already crossed the threshold and NO only after the full transcript at a stricter cap. Executed as fill-and-kill clips with a side-dependent price cap. 24 runs, 27 bets, 25 bets won (22 of the 24 markets, a position that expired worthless counts as lost). The measured lesson is negative and stated as such: the market reprices a spoken word in 1–4 seconds, the pipeline needs 15–25, so the single-word latency race is structurally lost — the edge that remained lives in doubt windows and count brackets, and the bot was still first taker on the traded side in 11 of 15 reconciled bets.
